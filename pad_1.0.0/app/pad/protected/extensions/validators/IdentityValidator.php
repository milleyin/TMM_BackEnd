 ...
 *
 */
class IdentityValidator extends CValidator
{
    /**
     * 容许空白的
     * @var unknown
	...
	protected function validateAttribute($object, $attribute)
	{
	    ...
	    if ( !preg_match('/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/', $object->$attribute))
			$this->addError($object, $attribute, '{attribute} 不是有效的值');
		elseif ( !self::isCard($object->$attribute))
			$this->addError($object, $attribute, '{attribute} 不是有效的值');
	}
}