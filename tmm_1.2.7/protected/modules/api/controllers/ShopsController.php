...
class ShopsController extends ApiController
{
	...
	public function actions()
	{
		...
		return array(
				//专题、 广告 列表页
				'column'=>'api.controllers.actions.shops.ColumnShopsAction',
				//微信定位
				'wxgps'=>'api.controllers.actions.shops.WxgpsShopsAction',
		);
		//{actions}
	}
	...
	{	
		...
		elseif($exist_City==true && $exist_Gps==true)
		{
			...
			{
				$select='ROUND(6378.138*2*ASIN(SQRT(POW(SIN(( :lat *PI()/180-`Pro_Items`.`lat`*PI()/180)/2) , 2)+COS( :lat *PI()/180)*COS(`Pro_Items`.`lat`*PI()/180)*POW(SIN(( :lng *PI()/180-`Pro_Items`.`lng`*PI()/180)/2),2)))*1000) AS distance';
	...
	public function list_data($models,$domain,$distance)
	{
		...
		foreach ($models as $key=>$model)
		{
			...
			$return[$key]=array(
					...
					'selected_info'=>CHtml::encode($model->Pro_Shops->selected_info),
