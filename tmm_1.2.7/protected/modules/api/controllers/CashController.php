...
class CashController extends ApiController
{
	...
	private function validate_account($model, $total_money='')
	{
		...
		if ($this->floorComp($total_money, Yii::app()->params['order_deposit_price_user']) == -1) 
		{
			$model->addError('msg', '提现金额不足'.Yii::app()->params['order_deposit_price_user'].'元');
			$model->addError('status', '002');
			return false;
		}
		if (Yii::app()->controller->floorComp($account_model->money, $total_money) == -1)
		{
