 ...
 *	创建时间：2016-04-05 14:17:43 */
class EatController extends OperatorMainController
{
	/**
	 * 默认操作数据模型
	 * @var string
	...
	public function actionView($id)
	{
		$model = $this->loadModel($id, array('with'=>array(
				'Eat_Items'=>array(
					'condition'=>'Eat_Items.status!=:status',
				...
				'Eat_ItemsClassliy'=>array('select'=>'name'),
				'Eat_Fare',
				'Eat_ItemsImg',
		)));
	...
	public function actionCreate($id)
	{
		...
		//提交表单 排除添加价格类型
        if (!isset($_POST['add']) && !isset($_POST['cut']) && isset($_POST['Items']) && 
        	  is_array($_POST['Fare']) && count($_POST['Fare']) == count($model->Eat_Fare)
        	)
        {
        	//赋值
            $model->Eat_Items->attributes = $_POST['Items'];
            //吃
            $model->Eat_Items->c_id = Items::items_eat;
			//归属运营商
            $model->Eat_Items->agent_id = Yii::app()->operator->id;
            //归属供应商
            $model->Eat_Items->store_id = $model->Eat_Items->Items_StoreContent->id;
            //项目字段验证
			...
			if ($model->Eat_Items->content == '')
			{
				$model->Eat_Items->addError('content', '详细内容 不可空白');
				$content_validate = false;
			}
			else
			{
		   ...
		   if ($Fare_validate && $Items_validate && $img_validate && $content_validate)
		   {
				...
				{
					//创建的项目是下线状态
					$model->Eat_Items->status = Items::status_offline;
					//创建的默认未提交
					$model->Eat_Items->audit = Items::audit_draft;
					//处理图片链接
					$model->Eat_Items->content = $this->admin_img_replace($model->Eat_Items->content);
					//地图的地址				
					$model->Eat_Items->map = $this->getFilePath(Yii::app()->params['uploads_items_map']).'.png';
					//保存项目主要表
						...
						if ($model->save(false))
						{
							...
							foreach ($_POST['ItemsImg']['tmp'] as $name)
							{
								...
								if ($this->file_exists_uploads($filename))
								{
									...
									//保存图片
									if ( !$this->items_img_save($model->Eat_Items, $filename))
										throw new Exception("添加项目(吃)图片记录错误");
								}
							}	
							$return = $this->log('添加项目(吃)', ManageLog::operator, ManageLog::create);
						}
						else
	...
	public function actionUpdate($id)
	{
		...
		$model = $this->loadModel($id, array('with'=>array(
				...
				'condition'=>'Eat_Items.status=:status AND t.c_id=:c_id AND Eat_Items.audit!=:audit AND Eat_Items.agent_id=:agent_id',
				'params'=>array(':c_id'=>Items::items_eat, ':status'=>Items::status_offline, ':audit'=>Items::audit_pending, ':agent_id'=>Yii::app()->operator->id),
		));
		//获取价格 对象中字段 ID
		$fare_ids = $this->listData($model->Eat_Fare, 'id', 'id');
		//获取概况图 对象中字段 ID
		$items_img_ids = $this->listData($model->Eat_ItemsImg, 'id', 'id');
		//设置 价格对象 场景
		...
		if ((isset($_POST['add']) || isset($_POST['cut'])) && isset($_POST['Items'],$_POST['Fare']) && is_array($_POST['Fare']))
		{
			...
			if (isset($_POST['Items']['free_status']) && $_POST['Items']['free_status'] != Items::free_status_yes)
			{
				...
				//设置属性 保存数据
				$array = $this->set_attributes($model->Eat_Fare, $default, $number);
				//原来的小 需要在添加对象
				if ($number > $count)
				{
			...
			)
		{
			...
			if($Fare_validate && $Items_validate && $items_img_validate && $content_validate)
			{
				...
				try
				{
					...
					{
						//保存图片
						if ($model->Eat_Items->map != $old_map)
						{
							Items::saveAmapImage($model->Eat_Items->map, $model->Eat_Items->lng, $model->Eat_Items->lat);
						...
						if ( !empty($fare_ids))
						{
							if ( !Fare::model()->deleteAll(array('condition'=>'id in (' . implode(',', $fare_ids)  . ')' )))
								throw new Exception("删除项目(吃)不用的价格记录错误");
						}
						if (isset($_POST['ItemsImg']['tmp']) && is_array($_POST['ItemsImg']['tmp']))
						{
							foreach ($_POST['ItemsImg']['tmp'] as $name)
						...
						$return = $this->log('修改项目(吃)', ManageLog::operator, ManageLog::update);
					}
					else
