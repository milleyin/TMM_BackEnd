...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<?php
		...
		echo $form->errorSummary(array($model->Eat_Items,$Eat_ItemsImg));
	?>
    <div class="row">
    ...
    <?php
	    if ($this->file_exists_uploads($model->Eat_Items->map))
		{
    ...
    <div class="row">
        ...
        <?php echo $form->dropDownList($model->Eat_Items, 'free_status', array(''=>'--请选择--')+Items::$_free_status, array(
        							'ajax'=>array(
        									'type'=>'POST',
    ...
    <div class="row" style="width:700px;" id="fare_list">
        ...
        <?php  
        	foreach ($model->Eat_Fare as $key=>$Eat_Fare)
        	{
        ?>
...
<script type="text/javascript">
    ...
    jQuery(function($){
        $("#<?php echo CHtml::activeId($Eat_ItemsImg, 'tmp') ?>").uploadify({
            ...
            'uploader': '<?php echo Yii::app()->createUrl('/operator/eat/uploads'); ?>',
            'auto': true,
            'multi': true,
		...
		function delete_file() {
			jQuery(".uploadify-queue-item a").on('click', function () {
				...
				jQuery.ajax({
					type: 'POST',
					url: '<?php echo Yii::app()->createUrl('/operator/eat/uploads');?>',
	...
	function uploadify_input(){
		jQuery("#uploadify_input").val(<?php echo 'jQuery("#'.CHtml::activeId($Eat_ItemsImg, 'tmp').'-queue").html()'; ?>);
	}
	//地图点击回调函数
