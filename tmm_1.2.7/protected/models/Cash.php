...
// 	const cash_type_admin=1;
	...
	public function rules()
	{
		...
		return array(
			...
			//提现
			array('price', 'required', 'on'=>'create'),
			array('price', 'safe', 'on'=>'create'),
			array('price', 'priceValidator', 'on'=>'create'),
			array('phone, sms, search_time_type, search_start_time, search_end_time, id, cash_type, cash_id, admin_id_first, remark_first, first_time, admin_id_double, remark_double, double_time, admin_id_submit, remark_submit, submit_time, money, fee_price, fact_price, bank_card_id, bank_id, bank_name, bank_branch, bank_code, bank_type, bank_identity, cash_status, audit_status, pay_type, pay_time, add_time, up_time, status', 'unsafe', 'on'=>'create'),
			
			//短信验证
			array('sms', 'required', 'on'=>'save'),
			array('sms', 'safe', 'on'=>'save'),
			array('search_time_type, search_start_time, search_end_time, id, cash_type, cash_id, admin_id_first, remark_first, first_time, admin_id_double, remark_double, double_time, admin_id_submit, remark_submit, submit_time, money, price, fee_price, fact_price, bank_card_id, bank_id, bank_name, bank_branch, bank_code, bank_type, bank_identity, cash_status, audit_status, pay_type, pay_time, add_time, up_time, status', 'unsafe', 'on'=>'save'),
				
			// 商家提现
			array('phone, sms, price', 'required', 'on'=>'store_create'),
			...
			// 用户提现
			array('phone, sms, price', 'required', 'on'=>'user_create'),
			...
			// 提现====验证价格
			array('price', 'required', 'on'=>'user_create'),
			...
			//operatorSearch
			array('search_time_type,search_start_time,search_end_time,id, money, price, fee_price, fact_price, bank_card_id, bank_id, bank_name, bank_branch, bank_code, bank_type, bank_identity, cash_status, audit_status, pay_type, pay_time, add_time, up_time, status', 'safe', 'on'=>'operatorSearch'),
		);
	}
	...
	public function authenticate_price()
	{
		...
		if ($floor->floorComp($this->money, $this->price) != 0)
			$this->addError('price','提现金额应等于 申请金额');
		if ($floor->floorComp($this->money, $this->fee_price) == -1)
			$this->addError('fee_price','提现费用不能大于 申请金额');
		if ($floor->floorComp($this->price, $this->fee_price) == -1)
	...
	public function relations()
	{
		...
		return array(
			...
			// 关联银行表
			'Cash_BankCard'=>array(self::BELONGS_TO, 'BankCard', 'bank_card_id'),
		);
	}
	...
	public function operatorSearch($criteria='')
	{
		...
		if ($criteria === '')
		{
			...
			$criteria->addColumnCondition(array(
				'`t`.`cash_type`'=>self::cash_type_agent,
				'`t`.`cash_id`'=>Yii::app()->operator->id,
			));
			$criteria->compare('t.id',$this->id,true);
			$criteria->compare('t.money',$this->money,true);
			$criteria->compare('t.fee_price',$this->fee_price,true);
	...
	{
		return self::getAuditRemark($log);
	}
	...
	/**
	 * 是否有提现中的
	 * @param unknown $cash_type
	 * @param unknown $cash_id
	 * @return Ambigous <NULL, unknown, multitype:unknown Ambigous <unknown, NULL> , mixed, multitype:, multitype:unknown >
	 */
	public static function getIsCash($cash_id, $cash_type)
	{
		$criteria = new CDbCriteria;
		$criteria->addColumnCondition(array(
			'cash_type'=>$cash_type,
			'cash_id'=>$cash_id,
			'cash_status'=>self::cash_status_cashing,
	 ...
	 * @param unknown $role
	 * @param unknown $phone
	 * @param unknown $coment
	 * @return boolean
	 */
	public function sendSms($sms, $role, $phone, $coment)
	{
		if (isset($sms['sms_id'], $sms['sms_type'], $role['role_id'], $role['role_type']))
		{
			...
			if (Send_sms::is_send($phone, $params_v, Yii::app()->params['sms'][$coment]['number'] ,Yii::app()->params['sms'][$coment]['interval']))
			{
				...
				$params = array(
						...
						'login_address'=>'',
						'sms_error'=>Yii::app()->params['sms'][$coment]['error'],
						'end_time'=>time()+Yii::app()->params['sms'][$coment]['time'],
				);
				if (Send_sms::send($phone, $params, strtr(Yii::app()->params['sms'][$coment]['content'], array('{code}'=>$code))))
					return true;
				else 
					$this->addError('price', '操作过于频繁，请稍后操作！');		
			}
			else
				$this->addError('price', '操作过于频繁，请稍后操作！');
		}
	 ...
	 * @return boolean
	 */
	public function validatorSms($sms, $role, $phone)
	{
		if (isset($sms['sms_id'], $sms['sms_type'], $role['role_id'], $role['role_type']))
		{
			Yii::import('ext.Send_sms.Send_sms');
			...
			if ( !Send_sms::verifycode($phone, $params, $this->sms))
	...
	public function priceValidator()
	{
		if ( !$this->hasErrors())
		{
			if ($this->accountModel)//最小
			{
				if (Yii::app()->controller->floorComp($this->price, $this->cash_min) == -1)
					$this->addError('price', '提现金额 最小为' . $this->cash_min);
				if (Yii::app()->controller->floorComp($this->accountModel->money, $this->price) == -1)
						$this->addError('price', '可用余额不足');
			}
			else
				$this->addError('price', '钱包 不存在'); 
		}
	}
	
	public static function setCash($model, $data, $cash, $cash_min, $default = '')
	{
		if (isset($cash['cash_id'], $cash['cash_type'], self::$_cash_type[$cash['cash_type']]))
		{
			$model->price = $default;
			$model->scenario = 'create';
			$model->attributes = $data;
			$model->cash_min = $cash_min;		
			$model->cash_id = $cash['cash_id'];
			$model->cash_type = $cash['cash_type'];
			$model->bank_card_id = $model->Cash_BankCard->id;
			$model->bank_id = $model->Cash_BankCard->bank_id;
			$model->bank_name = $model->Cash_BankCard->bank_name;
			$model->bank_branch = $model->Cash_BankCard->bank_branch;
			$model->bank_code = $model->Cash_BankCard->bank_code;
			$model->bank_type = $model->Cash_BankCard->bank_type;
			$model->bank_identity = $model->Cash_BankCard->bank_identity;
			return $model;
		}
		return null;
	}
	
	public static function cashSave($model)
	{
		$model->scenario = 'create';
		return $model->save();
	}
