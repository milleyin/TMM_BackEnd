...
// 			array('search_time_type,search_start_time,search_end_time,id, pro_id, group_id, items_id, thrand_id, add_time, up_time, status', 'unsafe', 'on'=>'create_thrand,update_thrand,create_actives,update_actives'),
			//线的创建 修改
			array('fare_id, items_id', 'required','on'=>'create_thrand, update_thrand'),
			array('fare_id, items_id', 'safe','on'=>'create_thrand, update_thrand'),
			array('fare_id', 'validatorFare_id', 'on'=>'create_thrand, update_thrand'),
			array('search_time_type,search_start_time,search_end_time,id, pro_id, group_id, thrand_id, add_time, up_time, status', 'unsafe', 'on'=>'create_thrand, update_thrand'),
	...
	/**
	 * 验证 选择项目价格 和项目 是否是有效值
	 */
	public function validatorFare_id()
	{
		if ( !$this->hasErrors())
		{
			$criteria = new CDbCriteria;
			$criteria->select = 'id';
			$criteria->with = array(
				'Fare_Items'=>array('select'=>'id')
			);
			$criteria->addColumnCondition(array(
					'`Fare_Items`.`audit`'=>Items::audit_pass,
					'`Fare_Items`.`status`'=>Items::status_online,
					'`t`.`item_id`'=>$this->items_id,
			));
			if ( !Fare::model()->findByPk($this->fare_id, $criteria))
				$this->addError('fare_id', '选择项目价格 不是有效值');
		}
	}
}
