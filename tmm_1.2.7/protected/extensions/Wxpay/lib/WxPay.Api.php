...
class WxPayApiApp extends WxPayApi
{
	/**
	 *
	 * 统一下单，WxPayUnifiedOrder中out_trade_no、body、total_fee、trade_type必填
	...
	{
		//如果不需要上报数据
		if(WxPayConfig::APP_REPORT_LEVENL == 0){
			return;
		}
		//如果仅失败上报
		if(WxPayConfig::APP_REPORT_LEVENL == 1 &&
			 array_key_exists("return_code", $data) &&
			 $data["return_code"] == "SUCCESS" &&
			 array_key_exists("result_code", $data) &&
			 $data["result_code"] == "SUCCESS")
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		//如果有配置代理这里就设置代理
		if(WxPayConfig::APP_CURL_PROXY_HOST != "0.0.0.0"
				&& WxPayConfig::APP_CURL_PROXY_PORT != 0){
			curl_setopt($ch,CURLOPT_PROXY, WxPayConfig::APP_CURL_PROXY_HOST);
			curl_setopt($ch,CURLOPT_PROXYPORT, WxPayConfig::APP_CURL_PROXY_PORT);
		}
		curl_setopt($ch,CURLOPT_URL, $url);
		curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,FALSE);
		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
