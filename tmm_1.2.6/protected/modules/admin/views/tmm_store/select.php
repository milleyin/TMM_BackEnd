...
$this->breadcrumbs=array(
	...
	$select->phone=>array('view','id'=>$select->id),
	'选择标签页'
);
...
");
?>
<h1>选择标签 <font color='#eb6100'><?php // echo $select->name; ?></font></h1>
<div>
	<span>
...
<?php
$this->renderPartial('/tmm_tags/_search',array(
	'model'=>$model,
));
?>
</div><!-- search-form -->
...
$this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns'=>array(
		array(
				...
				'name'=>'id',
				'id'=>'tags',
				'checked'=>'TagsElement::checked_element($data->id,'.$select->id.','.TagsElement::tags_store_content.')',
...
$url=Yii::app()->createUrl('/admin/tmm_store/tags',array('id'=>$select->id));

$ajax=<<<EOD
	//选中的tags
	function select_tags(){
	    return $('#tags-grid').yiiGridView('getChecked', 'tags');
	}
	...
	jQuery(document).on('change','#tags_all',function() {
		if($(this).is(':checked')){
			Checkbox(arr_dive(all_tags(),select_tags()),'yes');
		}else
