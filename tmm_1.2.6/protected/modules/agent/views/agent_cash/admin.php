...
<div class="content_box">
    ...
    <div class="query">
        ...
        <?php echo $form->dropDownList($model,'audit_status',array(''=>'订单状态')+$model::$_audit_status); ?>
...
$('.query form').submit(function(){
	$('#agent_cash_grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
function(){
	...
	jQuery(document).on('click','.stop_div .confirm',function(){
			...
			jQuery('#agent_cash_grid').yiiGridView('update',{
				type: 'POST',
    ...
    $this->widget('zii.widgets.grid.CGridView', array(
        ...
        'columns'=>array(
            ...
            array(
                'header'=>'结算状态',
                'name'=>'audit_status',
                'value'=>'Cash::$_audit_status[$data->audit_status]',
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:150px;'),
                'htmlOptions'=>array('style'=>'text-align:center;'),
            ),
            array(
                ...
                'buttons'=>array(
                    'view'=>array(
                        'label'=>'明细',
                        'imageUrl'=>false,
                        'url'=>'Yii::app()->createUrl("/agent/agent_bills/admin",array("id"=>$data->id))',
                    ),
                ),
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:200px;'),
            ),
