<div class="content_box" id="page_project_create">
    ...
    echo $this->breadcrumbs(array(
        ...
        '项目创建',
    	$model->Eat_Items->Items_Store_Manager->phone,
    ));
    ?>
	<!--.title-->
		...
		<?php 
			...
			?>
				<div class="row-fluid project_type_row-fluid">
					...
					?>
						<option <?php echo $model->Eat_Items->c_id==$ItemsClassliy->id?'selected="selected"':'';?> title="<?php echo CHtml::encode($ItemsClassliy->info);?>" value="<?php echo Yii::app()->createUrl('/agent/agent_'.$ItemsClassliy->admin.'/create',array('id'=>$model->Eat_Items->Items_Store_Manager->id));?>">
	...
	<?php
        ...
        ?><?php echo $form->error($model->Eat_Items,'start_work'); ?></div>
					<div class="division">
					</div>
					<div class="col-stop">
        <?php
        ...
        ?><?php echo $form->error($model->Eat_Items,'end_work'); ?></div>
				</div>
