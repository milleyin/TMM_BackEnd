<div class="container-fluid content_box scenic_spot" id="pageone">
    <?php
            ...
            <div class="content content_one">
                ...
                ?>
                        <select class="span2" name="search_status">
                            <option value="<?php echo Items::status_del ?>">全部</option>
                            <option value="<?php echo Items::status_online ?>">已上线</option>
                            <option value="<?php echo Items::status_offline ?>">未上线</option>
                        </select>
                ...
                <?php $this->endWidget(); ?>
                <?php echo CHtml::link('创建项目',array('/agent/agent_items/select'),array('class'=>'create pull-right'))?>
                </div>  <!-- .query -->
                <!-- .main_content -->
                <div class="main_content data" id="masonry">
<?php
...
jQuery(document).on('click','#items_list a.start',function() {
    if(!confirm('确定要上线此项目吗?')) return false;
    var th = this,
        afterDelete = function(){};
    $.fn.yiiListView.update('items_list', {
        type: 'POST',
...
jQuery(document).on('click','#items_list a.disable',function() {
    if(!confirm('确定要下线此项目吗?')) return false;
    var th = this,
        afterDelete = function(){};
    $.fn.yiiListView.update('items_list', {
        type: 'POST',
