...
class CThumb extends CApplicationComponent {
    ...
    public function init() {
        ...
        if (!function_exists("imagecreatetruecolor")) {
            throw new CHttpException(500,"使用这个类,需要启用GD库");
        }
        parent::init();
    }
    ...
    private function loadImage() {
        if (!$this->image) {
            throw new CHttpException(500,"必须有图片名称");
        }
        ...
        switch ($this->srcType) {
            ...
            case IMAGETYPE_PNG:
                ...
                break;
            default:
            	throw new CHttpException(500,'不支持的图像类型');
        }
    }
    ...
    public function save() {
        if (!$this->directory) {
            throw new CHttpException(500,"输入保存缩略图目录");
        }
          switch ($this->srcType) {
	          case IMAGETYPE_JPEG:
