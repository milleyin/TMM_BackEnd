...
foreach ($this->tableSchema->columns as $column)
{
    ...
    if (strpos($column->name,'time') !== false)
    {
        echo "\t\tarray(
                ...
                'htmlOptions'=>array('style'=>'text-align:center;', 'title'=>function (\$row, \$data) {
                        return \$data->getAttributeLabel('$column->name') . '：' . Yii::app()->format->FormatDate(\$data->$column->name);
                    },
                ),
        ),\n";
    }
    elseif (strpos($column->name, 'status') !== false || strpos($column->name, 'type') !== false )
    {
        echo "\t\tarray(
                ...
                'value'=>function (\$data, \$row) {
                    return \$data::\$_$column->name[\$data->$column->name];
                },
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:50px;'),
                'htmlOptions'=>array('style'=>'text-align:center;', 'title'=>function (\$row, \$data) {
                        return \$data->getAttributeLabel('$column->name') . '：' . \$data::\$_$column->name[\$data->$column->name];
                    },
                ),
        ),\n";
    }
    ...
    else
    {
        echo "\t\tarray(
                ...
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:50px;'),
                'htmlOptions'=>array('style'=>'text-align:center;', 'title'=>function (\$row, \$data) {
                        return \$data->getAttributeLabel('$column->name') . '：' . \$data->$column->name;
                    }
                ),
        ),\n";
    }
...
?>
        array(
            'class'=>'CButtonColumn',
            'header'=>'操 作',
            'template'=>'{view}{update}<?php echo isset($is_audit) ? '{audit}' : ''; ?>{delete}{disable}{start}{restore}',
            'buttons'=>array(
                    'view'=>array(
...
?>
                    'audit'=>array(
                        ...
                        'url'=>function ($data, $row) {
                            return  CHtml::normalizeUrl(array('audit', 'id'=>$data->id));
                            //return Yii::app()->createUrl('<?php echo $this->controller;?>/audit', array('id'=>$data->id));
                        },
                        'options'=>array('style'=>'padding:0 8px 0 0;'),
                    ),
<?php }?>
                    ...
                    'disable'=>array(
                        ...
                        'url'=>function ($data, $row) {
                            ...
                            //return Yii::app()->createUrl('/<?php echo $this->controller?>/disable', array('id'=>$data->id));
                        },
                        'click'=>$click,
                        'options'=>array('style'=>'padding:0 8px 0 0;'),
                    ),
                    'start'=>array(
                        ...
                        'url'=>function ($data, $row) {
                            ...
                            //return Yii::app()->createUrl('/<?php echo $this->controller?>/start', array('id'=>$data->id));
                        },
                        'click'=>$click,
                        'options'=>array('style'=>'padding:0 8px 0 0;'),
                    ),
                    'restore'=>array(
                        ...
                        'url'=>function ($data, $row) {
                            ...
                            //return Yii::app()->createUrl('/<?php echo $this->controller?>/restore', array('id'=>$data->id));
                        },
                        'click'=>$click,
                        'options'=>array('style'=>'padding:0 8px 0 0;'),
                    ),
