...
class PadController extends AdminModulesController
{
    ...
    public function actionAdmin()
    {
        ...
        $model->Pad_Store = new \Store('search');
        //清除默认值
        $model->unsetAttributes();
        $model->Pad_Store->unsetAttributes();
        if (isset($_GET['Pad']))
            $model->attributes = $_GET['Pad'];
        if (isset($_GET['Store']))
            $model->Pad_Store->attributes = $_GET['Store'];
    ...
    public function actionSelect($id)
    {
        ...
        $model->status = '=' . \Pad::_STATUS_NORMAL;
        $model->Pad_Store->status = '=' . \Store::_STATUS_NORMAL;
        
        $this->_modelName = 'Ad';
        $this->render('select', array(
                'model'=>$model,
                'select'=>$this->loadModelByPk($id, 'status=:status', array(':status'=>\Ad::_STATUS_NORMAL)),
        ));
    }
    ...
    public function actionRestore($id)
    {
        ...
        if ( !($padModel = $this->loadModel('number=:number AND status!=:status', array(':number'=>$model->number, ':status'=>\Pad::_STATUS_DELETED))))
        {
            $transaction = $model->dbConnection->beginTransaction();
            try
            {
