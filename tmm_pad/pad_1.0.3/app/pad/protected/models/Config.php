...
class Config extends ActiveRecord
{
     ...
     * @var integer
     */
    const Config_TYPE_FREE = 0;
    /**
     * 配置类型 付费
     * @var integer
     */
    const Config_TYPE_PAY = 1;
    /**
     * 解释字段 type 的含义
     * @var array
     */
    public static $_type = array(
        self::Config_TYPE_FREE => '免费',
        self::Config_TYPE_PAY => '付费',
    );
    /**
     * 解释字段 status 的含义
     * @var array
    ...
    public function rules()
    {
        ...
        return array(
            array('type, status, chance_number', 'numerical', 'integerOnly'=>true),
            array('store_id, pad_id, manager_id, money', 'length', 'max'=>20),
            array('chance_number, number', 'length', 'max'=>11),
            array('info, ad_url', 'length', 'max'=>128),
            ...
            array('id, store_id, pad_id, manager_id, type, chance_number, number, money, info, ad_url, up_time, add_time, status', 'safe', 'on'=>'search'),
        );
    }
    ...
    public function search()
    {
        ...
        $criteria->compare('t.ad_url', $this->ad_url,true);
     ...
     * 检测类型和付费金额
     */
    public function typeMoneyValidator()
    {
        if ( !$this->hasErrors()) {
            if ($this->type == self::Config_TYPE_FREE && bccomp($this->money, 0, 2) !== 0) {
                $this->addError('money', '付费金额必须等于零');
            } elseif ($this->type == self::Config_TYPE_PAY && bccomp($this->money, 0, 2) !== 1) {
                $this->addError('money', '付费金额必须大于零');
            }
        }
    }
}