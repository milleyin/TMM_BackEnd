...
class CThumb {
    ...
    public function __construct() {
        ...
        if (!function_exists("imagecreatetruecolor")) {
            ...
            throw new Exception(500,"使用这个类,需要启用GD库");
        }
    }

    public function setSrcExt($val) {
        $this->srcExt = $val;
    }
    ...
    private function loadImage() {
        if (!$this->image) {
            throw new Exception(500,"必须有图片名称");
        }
        ...
        switch ($this->srcType) {
            ...
            case IMAGETYPE_PNG:
                ...
                break;
            default:
                throw new Exception(500,'不支持的图像类型');
        }
    }
    ...
    public function alpha() {
        imagealphablending($this->thumbImage,false);//这里很重要,意思是不合并颜色,直接用$img图像颜色替换,包括透明色;
        imagesavealpha($this->thumbImage,true);//这里很重要,意思是不要丢了$thumb图像的透明色;
    }

    public function save() {
        if (!$this->directory) {
            throw new Exception(500,"输入保存缩略图目录");
        }
        switch ($this->srcType) {
            case IMAGETYPE_JPEG:
