...
class CallbackController extends ApiController
{
     ...
     * @var boolean
     */
    public $enableCsrfValidation = array('alipay' => false, 'alipayapp' => false, 'wxpay' => false);
    
    public function actions()
    {
        //{actions}
        return array(
            'wxpay'=>'api.controllers.actions.callback.WxpayCallbackAction',
            'wxpayapp'=>'api.controllers.actions.callback.WxpayappCallbackAction',
        );
        //{actions}
    }
     ...
     */
    public function notify_data($data, $online=Order::pay_type_alipay)
    {    
        //商户订单号====系统唯一订单号
        ...
        if ( !$model)                                                                                                    //订单不存
            return false;
        //$data['total_fee'] = $model->order_price;
        //开启事物
        ...
        try{
                ...
                )                                                                                                                //订单状态 待支付
            {
                ...
                if($return)
                    self::globalErrorRecord(Order::$_pay_type[$online] . '--success', '回调信息====== 没处理 保存成功' ,'api');
                else
            ...
            {            
                //已经支付的操作
                $return = true;
                self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调信息====== 已处理 保存成功' ,'api');
            }
            else
            {
                $return = false; //失败
                self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调信息======订单状态异常'.$order->order_no ,'api');
        ...
        catch (Exception $e)
        {
            ...
            self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调订单事物回滚：'.$e->getMessage(),'api');
        }
        return $return;
    }
        ...
        if ($model->pay_status==Order::pay_status_not || $model->pay_status==Order::pay_status_paying)
        {
            ...
            if ($return)
            {
                if ($model->order_type == Order::order_type_dot)                                        //点
                {
                    ...
                    $valid = $valid ? '成功' : '失败';
                    self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调信息====将扫描二维码的值改为可用'.$valid ,'api');
                    ...
                    if (! empty($items_is_barcode))
                    {
                        if (OrderItems::set_is_barcode($items_is_barcode))
                            self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调信息保存成功 项目免费====消费码设为无效码 OrderItems ID:'.implode(',', $items_is_barcode) ,'api');
                    }
                    //在线支付
                    if ($online == Order::pay_type_wxpay || $online == Order::pay_type_alipay)
                    {
                        //交易记录 支付宝在线支付
                        ...
                        $account_log = Account::moneyRecordAlipayRmb(
                                ...
                                array('info_id'=>$model->id,'name'=>Order::$_pay_type[$online] . '在线支付','address'=>'','info'=>
                                    $user->nickname.'（'.$user->phone.'）'."<br>\n".
                                    '觅境订单 '.$model->order_no."<br>\n".
                                    '支付金额 '.$data['total_fee']."<br>\n".
                                    '出游日期 '.date('Y-m-d',$model->go_time)."<br>\n".
                                    date('Y-m-d H:i:s'). Order::$_pay_type[$online] . '在线支付成功（手机支付）'
                                )
                        );
                        if (! $account_log)
                ...
                {
                    self::globalErrorRecord(Order::$_pay_type[$online] . '--fail','回调信息====没有找到订单' ,'api');
                    return false;            
                }

                self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调信息====保存成功' ,'api');            
                ...
                self::globalErrorRecord(Order::$_pay_type[$online] . '--success','回调信息====通知用户'.$sms_content ,'api');
                return true;
            }else{
                //发邮件  ====通知管理员  回调成功，处理失败
                self::globalErrorRecord(Order::$_pay_type[$online] . '--fail','回调信息====保存失败' ,'api');
                return false;
            }
        }else 
            return true;
    ...
    public function account($id,$return)
    {
        ...
        if($order)
        {
            ...
            if($user_account && $user_account->status == Account::status_normal && $this->floorComp($user_account->money,$order->order_price) >=0)
            {
                $data = array(
                ...
                if($this->notify_data($data, Order::pay_type_account))
                {
                    $return['status']=array(
                            'name'=>'支付成功',
                            'value'=>1,
