...
class AdminController extends Controller {
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        if(empty($menus)){
            ...
            //高亮主菜单
            $current = M('Menu')->where("url like '%{$controller}/".ACTION_NAME."%'")->order('sort asc')->field('id')->find();
            if($current){
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if($item['title'] == $nav_first_title){
                        ...
                        //生成child树
                        $groups = M('Menu')->where("pid = {$item['id']}")->order('sort asc')->distinct(true)->field("`group`")->select();
                        if($groups){
