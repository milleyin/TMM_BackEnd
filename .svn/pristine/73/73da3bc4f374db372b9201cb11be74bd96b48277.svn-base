...
class DocumentModel extends RelationModel{
	...
	protected $_link = array(
		'ArticleCover' => array(
			...
			'class_name'     => 'Picture',
			'mapping_fields' => 'path as cover_path',
			'foreign_key'    => 'cover_id',
			'as_fields'      => 'cover_path',
		),
		'ArticleCoverDetail' => array(
			// 文章详情封面
			'mapping_type'   => self::BELONGS_TO,
			'class_name'     => 'Picture',
			'mapping_fields' => 'path as cover_detail_path',
			'foreign_key'    => 'cover_detail_id',
			'as_fields'      => 'cover_detail_path',
		),
		'ArticleCategory' => array(
			// 文章类别
			'mapping_type'   => self::BELONGS_TO,
			'class_name'     => 'Category',
			'mapping_fields' => 'title as category_name',
	...
	public function lists($category, $order = '`id` DESC', $status = 1, $field = true){
        // 推荐位为首页推荐
		$map = $this->listMap($category, $status, 4);
