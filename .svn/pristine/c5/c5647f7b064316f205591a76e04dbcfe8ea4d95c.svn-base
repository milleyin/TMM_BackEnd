...
class Tmm_playController extends MainController
{
    ...
    public function actionCreate($id)
    {
        ...
        $model->Play_Items->Items_StoreContent = $this->loadModel($id, array(
                'with'=>array(
                        'Content_Store'=>array(
                                'with'=>array('Store_Agent'),
                                'condition'=>'Store_Agent.status=:status_agent AND Content_Store.status=:status_store',
                                'params'=>array(':status_agent'=>Agent::status_suc, ':status_store'=>StoreUser::status_suc),
                        ),
                        'Content_Stoer_Son',
                ),
        ));
    ...
    public function actionUploads()
    {
        ...
        $uploads = array('tmp');
        //清空图片缓存
        $this->clear_tmp(Yii::app()->params['uploads_items_tmp_play']);
    ...
    public function actionUpdate($id)
    {
        ...
        $model = $this->loadModel($id, array('with'=>array(
                ...
                'condition'=>'Play_Items.status=:status AND t.c_id=:c_id AND Play_Items.audit !=:audit',
                'params'=>array(':status'=>Items::status_offline, ':c_id'=>Items::items_play, ':audit'=>Items::audit_pending,),
        ));
        //获取价格 对象中字段 ID
            ...
            if ($Fare_validate && $Items_validate && $items_img_validate && $content_validate)
            {
                ...
                try
                {
                    ...
                    if ($model->Play_Items->save(false))
                    {
                        ...
                        $return = $this->log('修改项目(玩)', ManageLog::admin, ManageLog::update);
                    }
                    else
