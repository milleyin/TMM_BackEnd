...
class SiteController extends Controller
{
	...
	public function actionShare()
	{
		...
		// 接收值 是否为数字
		if( !(is_numeric($share) && is_numeric($share) ) )
			throw new CHttpException($this->error_share,'分享参数 不是有效值');

		//根据类型分别调用
		switch($type){
			case $this->type_shops :
				$this->share_shops($share);break;
			case $this->type_items :
				$this->share_items($share);break;
			case $this->type_links :
				$this->share_links($share);break;
			default :
	...
	private function share_shops($share){
		...
		$model = Shops::model()->findByPk($share,'status=1');
