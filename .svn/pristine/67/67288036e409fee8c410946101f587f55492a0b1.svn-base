...
//        $this->wechatid = 'o_CytuIwPE8e5yHoLQgPzXgRePTMF';
    }
    ...
    /**
     * 投票
     * return 0投票失败 1首次投票成功 2投票成功  3未关注公众号
     */
    public function vote() {
        ...
        // 判断是否第一次投票
        $voteRacingInfo = $voteLogModel->where(array('from_id' => $this->wechatid))->order('add_time desc')->find();

        // 每天每人对同一参赛者只能投一票
        $condition['to_id'] = $voteLogData['to_id'];
        $condition['from_id'] = $this->wechatid;
        $condition['add_time'] = array('between', array(strtotime(date('Y-m-d', time()) . '00:00:00'), strtotime(date('Y-m-d', time()) . '23:59:59')));
    ...
    private function _racingList($type = 1) {
        ...
        $count      = $racingModel->where(array('audit_status'=>1, 'status'=>1))->count();// 查询满足要求的总记录数
    ...
    private function _rankInfo($id) {
        ...
        // poll票数 rank名次
        $sql = "SELECT * FROM (SELECT id,poll,(@rank:=@rank+1) AS rank FROM __LZ_RACING__,(SELECT (@rank:=0)) b WHERE audit_status=1 ORDER BY poll DESC, id DESC) c WHERE id={$id}";
        // 返回的是一个二维数组，取第一维
        $rankInfo = $model->query($sql);
        $rank = intval($rankInfo[0]['rank']);

        // 距上一名还差多少票
        $sql = "SELECT * FROM (SELECT id,poll,(@rank:=@rank+1) AS rank FROM __LZ_RACING__,(SELECT (@rank:=0)) b WHERE audit_status=1 ORDER BY poll DESC, id DESC) c WHERE rank=".($rank - 1);
