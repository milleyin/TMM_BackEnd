					...
					foreach ($pro->Pro_Items->Items_ItemsImg as $img)
					{
						if ($this->file_exists_uploads($img->img))
						{
							echo Yii::app()->controller->show_img($img->img, '', '', array(
									'title'=>Yii::app()->controller->setHideKey($pro->Pro_Items->Items_StoreContent->Content_Store->phone) . ' [' . 
