...
class CompatibleCommand  extends ConsoleCommand
{
	...
	{
// 		$models = User::model()->findAll('status=1');
// 		foreach ($models as $model)
// 		{
// 			$transaction=Yii::app()->db->beginTransaction();
// 			try {
// 				$return=Account::moneyEntryRechargeRmb(
// 						rand(100000, 999999) . '.' .rand(0, 99),
// 						array('account_id'=>$model->id, 'account_type'=>Account::user),
// 						array('info_id'=>$model->id, 'info'=>'在线充值', 'name'=>'在线充值', 'address'=>'')
// 				);
// 				if(! $return)
