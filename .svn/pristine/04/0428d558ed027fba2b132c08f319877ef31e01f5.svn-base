...
class Thrand extends CActiveRecord
{
							...
							foreach ($items_id_fare_key as $items_id=>$fare_key_fare_id)//
							{
								...
								foreach ($fare_key_fare_id as $fare_key=>$fare_id_price_number)
								{
									...
									foreach ($fare_id_price_number as $fare_id=>$price_number)
									{
										...
										//价格信息验证		
										if(!$floor->isNumeric($price_number['number']) || $price_number['number'] < 0 || $floor->floorComp($price_number['price'], $price_info['price']) != 0)
										{
											$model->addError('order_price','订单中商品 不是有效值');
											$model->addError('status','T27');
											return false;
										}
										if( $floor->floorComp($floor->floorMul($price_number['number'], $price_number['price']), $price_number['count']) != 0)
										{
											$model->addError('order_price','订单中商品 不是有效值');
										...
										//记录项目的总价
										Order::$order_items_money = $floor->floorAdd(Order::$order_items_money, $price_number['count']); 
	...
	public static function shops_price_num($id)
	{
		...
		foreach ($models as $model)
		{
			foreach ($model->Pro_ProFare as $fare)
				$num = $floor->floorAdd($num, $fare->ProFare_Fare->price);
		}
		return $num;
	}
	 ...
	 * @param unknown $data
	 * @return multitype:|Ambigous <multitype:, multitype:number unknown multitype:multitype:unknown   >
	 */
	public function dataArray($data)
	{
		...
		if(isset($data['Pro']))
		{
			...
			{
				$this->Thrand_Shops->addError('name', '线路天数 不可空白');
				return array();
			}
			//排序
			ksort($data['Pro']);
			foreach ($data['Pro'] as $day_sort => $half_sort_dot_ids)
			{
				if ( !is_array($half_sort_dot_ids))
				{
					$this->Thrand_Shops->addError('name', '天数 不可空白');
					return array();
				}
				//天 从一开始 不超过 配置的天数 默认十天
				if ( !(is_numeric($day_sort) && $day_sort >= 1 && ceil($day_sort / 2)<= Yii::app()->params['shops_thrand_day_number']))
				{
					$this->Thrand_Shops->addError('name', '线路天数 最大为' . Yii::app()->params['shops_thrand_day_number']);
					return array();
				}
				//排序
				ksort($half_sort_dot_ids);
				//点的顺序
				$half_sort = 0;
				foreach ($half_sort_dot_ids as $dot_ids_sort)
				{
					if ( !is_array($dot_ids_sort))
					{
						$this->Thrand_Shops->addError('name', '景点 不可空白');
						return array();
					}
					//一条线的一天最多可添加5个点 默认5
					if ($half_sort >= Yii::app()->params['shops_thrand_one_day_dot_number'])
					{
						$this->Thrand_Shops->addError('name', '一天最多能选中'. Yii::app()->params['shops_thrand_one_day_dot_number'] .'个景点');
						return array();
					}
					foreach ($dot_ids_sort as $dot_id => $sort_items_ids)
					{
						if ( !is_array($sort_items_ids))
						{
							$this->Thrand_Shops->addError('name', '项目 不可空白');
							return array();
						}
						//一个点中的项目 只能选择一次 不可重复
						$sort_items_ids = array_flip(array_flip($sort_items_ids));
						//排序
						ksort($sort_items_ids);
						//项目排序
						$sort = 0;
						foreach ($sort_items_ids as $items_id)
						{
							if (! isset($data['ProFare'][$day_sort][$half_sort][$dot_id][$sort][$items_id]))
							{
								$this->Thrand_Shops->addError('name', '项目价格 不可空白');
								return array();
							}
							$fare_ids = $data['ProFare'][$day_sort][$half_sort][$dot_id][$sort][$items_id];
							if ( !is_array($fare_ids))
							{
								$this->Thrand_Shops->addError('name', '项目价格 不可空白');
								return array();
							}
							$ProFare = array();
							//一个项目中价格 只能选择一次 不可重复
							$fare_ids = array_flip(array_flip($fare_ids));
							//排序
							ksort($fare_ids);
							foreach ($fare_ids as $fare_id)
							{
								$ProFare[] = array(
									'fare_id'=>$fare_id,
									'items_id'=>$items_id,
