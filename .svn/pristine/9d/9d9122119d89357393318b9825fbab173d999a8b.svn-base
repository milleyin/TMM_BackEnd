...
echo CHtml::beginForm(array('shop/sort'), 'post');
    ...
    'columns'=>array(
        ...
        array(
            ...
            'value'=>function ($data, $row) {
                return CHtml::textField('name[' . $data->id . ']', $data->sort, array('class'=>'name', 'maxlength'=>10, 'style'=>'
                        height: 24px;font-size: 16px;padding: 0 0 0 5px;border-radius: 5px;background-color: #EDE6E6;width:90px;
                '));
            },
        ...
        array(
                ...
                'type'=>'raw',
                'value'=>function ($data, $row) {
                        return CHtml::image($data->Shop_Upload->getUrlPath(), CHtml::encode($data->Shop_Upload->info), array('style'=>'width:80px;height:60px;'));
                },
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:100px;'),
                'htmlOptions'=>array('style'=>'text-align:center;', 'title'=>function ($row, $data) {
                          return $data->getAttributeLabel('Shop_Upload.size') . '：' . $data->Shop_Upload->getSize() . "\n" .
                            $data->getAttributeLabel('Shop_Upload.info') . '：' . $data->Shop_Upload->info;
                    },
                ),
        ),
//         array(
...
<div class="row">
    ...
    <span class="hint">默认500，倒序排序（相等更新时间倒序排序）。</span>
</div>
<?php 
    echo CHtml::endForm();
?>
