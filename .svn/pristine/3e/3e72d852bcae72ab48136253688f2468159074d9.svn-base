...
$this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns'=>array(
		array(
				...
				'buttons'=>array(
						'view'=>array(
							...
							'url'=>function ($data){
								$return = array();
								$return['link'] = Yii::app()->createUrl("/operator/thrand/viewdot",array("id"=>$data->id));
		...
		array(
			'class'=>'DataColumn',
			'evaluateHtmlOptions'=>true,
			'filter'=>CHtml::activeDropDownList($dotModel->Dot_Shops, 'agent_id', array(''=>'', '=' . Yii::app()->operator->id =>'自己的', '<>' . Yii::app()->operator->id => '其他人的'), array('id'=>false)),
			'name'=>'Dot_Shops.agent_id',
			'value'=>'Yii::app()->controller->setHideKey($data->Dot_Shops->Shops_Agent->phone)',
