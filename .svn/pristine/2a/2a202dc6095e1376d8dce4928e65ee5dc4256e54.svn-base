...
//				echo '审核通过失败！';
	...
	public function actionStart($id)
	{
		...
		exec(Yii::app()->basePath.'/./yiic actives end');
		//获取项目类型
	...
	public function actionDelete($id)
	{
		...
		$criteria->addColumnCondition(array(
			'`t`.`status`'=>Actives::status_published,
			'`Actives_Shops`.`status`'=>Shops::status_offline,
		));
		if ($this->loadModel($id, $criteria) && Shops::model()->updateByPk($id, array('status'=>Shops::status_del)))
			$this->log('删除觅趣活动',ManageLog::admin,ManageLog::update);
...
//		$this->_Ajax_Verify(array($model,$model->Actives_Shops),'actives-form');
	...
	public function actionUpdate($id)
	{
		...
		if ($model)
		{
			$update = $model->tour_type ==Actives::tour_type_dot ? 'dot_update' : 'thrand_update';
