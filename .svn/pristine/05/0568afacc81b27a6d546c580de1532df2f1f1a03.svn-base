   ...
   /**
    ...
    public function actionUpdate($id)
    {
        ...
        $model = $this->loadModel($id, array('with'=>array(
                ...
                'condition'=>'Eat_Items.status=:status AND t.c_id=:c_id AND Eat_Items.audit!=:audit',
                'params'=>array(':c_id'=>Items::items_eat, ':status'=>Items::status_offline, ':audit'=>Items::audit_pending),
        ));    
        //获取价格 对象中字段 ID
            ...
            if($Fare_validate && $Items_validate && $items_img_validate && $content_validate)
            {
                ...
                try
                {
                    ...
                    if ($model->Eat_Items->save(false))
                    {
                        ...
                        $return = $this->log('修改项目(吃)', ManageLog::admin, ManageLog::update);
                    }
                    else
