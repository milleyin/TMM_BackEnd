 <div class="content_box page_project_redact page_project_redact_again" id="page_project_create">
    ...
    <!--.title-->
    <?php 
    	if($model->Hotel_Items->audit==Items::audit_nopass){
    ?>
    <div class="box check_failed">
      <div><span>审核不通过原因</span></div>
      <div><span><?php echo AuditLog::get_audit_log(AuditLog::items_hotel,$model->id,AuditLog::nopass)->info;?></span></div>
    </div>
    <?php 
		}
	?>
    ...
    <div class="content">
      ...
      <?php 
	      if(isset($model->Hotel_ItemsImg[0]))
			  $Hotel_ItemsImg=$model->Hotel_ItemsImg[0];
	      else
			  $Hotel_ItemsImg=new ItemsImg;
      ?>
      <?php 
...
<script type="text/javascript">
	...
	$nunber=Yii::app()->params['items_image_number']-count($model->Hotel_ItemsImg);
