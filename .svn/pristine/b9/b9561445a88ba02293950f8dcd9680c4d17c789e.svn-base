...
<div>
    ...
    <span>
        <?php echo CHtml::link('创建广告', array('create')); ?>    
    </span>
</div>
...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    'columns'=>array(
        ...
        array(
                'class'=>'DataColumn',
                'evaluateHtmlOptions'=>true,
                'filter'=>CHtml::activeTextField($model->Ad_Upload, 'path', array('id'=>false)),
                'name'=>'Ad_Upload.path',
                'type'=>'raw',
                'value'=>function ($data, $row) {
                    if ($data->Ad_Upload->upload_type == Upload::UPLOAD_UPLOAD_TYPE_IMAGE)
                      return CHtml::image($data->Ad_Upload->getUrlPath(), CHtml::encode($data->Ad_Upload->info), array('title'=>$data->Ad_Upload->info,'style'=>'width:80px;height:60px;'));
                    return CHtml::link(CHtml::encode($data->name), $data->Ad_Upload->getUrlPath(), array('target'=>'_blank'));
                 },
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:150px;'),
                'htmlOptions'=>array('style'=>'text-align:center;', 'title'=>function ($row, $data) {
                       return $data->getAttributeLabel('Ad_Upload.info') . '：' . $data->Ad_Upload->info;
                   },
                ),
        ),
        array(
                'class'=>'DataColumn',
                'evaluateHtmlOptions'=>true,
                'filter'=>CHtml::activeTextField($model->Ad_Upload, 'size', array('id'=>false)),
                'name'=>'Ad_Upload.size',
                'value'=>function ($data, $row) {
                    return $data->Ad_Upload->getSize();
                },
                'headerHtmlOptions'=>array('style'=>'text-align:center;width:80px;'),
                'htmlOptions'=>array('style'=>'text-align:center;', 'title'=>function ($row, $data) {
                      return $data->getAttributeLabel('Ad_Upload.size') . '：' . $data->Ad_Upload->getSize();
                  },
                ),
        ),
        array(
        ...
        array(
            'class'=>'CButtonColumn',
            'header'=>'操 作',
            'template'=>'{view}{update}{select}{delete}{disable}{start}{restore}',
            'buttons'=>array(
                    'view'=>array(
                        'options'=>array('style'=>'padding:0 12px 0 0;'),
                    ),
                    'update'=>array(
                        'options'=>array('style'=>'padding:0 12px 0 0;'),
                    ),
                    'select'=>array(
                        ...
                        'url'=>function ($data, $row) {
                            return  CHtml::normalizeUrl(array('pad/select', 'id'=>$data->id));
                        },
                        'options'=>array('style'=>'padding:0 12px 0 0;'),
                    ),
                    'delete'=>array(
