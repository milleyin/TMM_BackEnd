...
class Chance extends ActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            // 添加抽奖机会
            array('user_id, store_id, pad_id, config_id, count, number', 'required', 'on'=>'create'),
            array('date_time', 'ext.validators.DefaultValueValidator', 'value'=>strtotime(date('Y-m-d', time())), 'on'=>'create'),
            array('pad_id', 'isPadIdValidator', 'on'=>'create'),
            array('user_id, store_id, pad_id, config_id, count, number', 'safe', 'on'=>'create'),
            array('id, date_time, up_time, add_time', 'unsafe', 'on'=>'create'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, user_id, store_id, pad_id, config_id, count, number, date_time, up_time, add_time, status', 'safe', 'on'=>'search'),
        );
    }
    ...
    public function search()
    {
        ...
        $this->timeSearch('t.date_time', $criteria);
    ...
    public function isPadIdValidator()
    {
        if ( !$this->hasErrors())
        {
            ...
            if (self::model()->find($criteria))
                $this->addError('pad_id', '已经获取过抽奖机会');
        }
    }
}