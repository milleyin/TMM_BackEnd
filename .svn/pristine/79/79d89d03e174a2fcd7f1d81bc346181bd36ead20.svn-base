 ...
 *	创建时间：2015-08-05 14:41:10 */
class Tmm_playController extends MainController
{
	/**
	 * 默认操作数据模型
	 * @var string
	...
	public function actionCreate($id)
	{
		...
		if(!isset($_POST['add']) && !isset($_POST['cut']) && isset($_POST['Items']) && isset($_POST['Fare']) && is_array($_POST['Fare']) && count($_POST['Fare'])==count($model->Play_Fare))
		{
			$model->Play_Items->attributes=$_POST['Items'];
			// 获取项目类型
			...
			$model->Play_Items->agent_id=$model->Play_Items->Items_StoreContent->Content_Store->agent_id;
			$model->Play_Items->store_id=$model->Play_Items->Items_StoreContent->id;
			...
			if($Fare_validate && $Items_validate){
				...
				try
				{
					...
					$model->Play_Items->map=$this->getFilePath().'.png';
					if($model->Play_Items->save(false))//保存项目主要表
					{
						...
						if(!$model->save(false))
							...
							if(!isset($Fare_is_error) && ((isset($img_is_error) && $img_is_error==false) || !isset($img_is_error))){
								...
								$return =$this->log('添加项目(玩)',ManageLog::admin,ManageLog::create);
							}else
									throw new Exception("添加项目(玩)中出现记录错误");
						}
					}else
	...
	public function actionUploads(){
		...
		$uploads=array('tmp');
		$this->clear_tmp(Yii::app()->params['uploads_items_tmp_play']);//清空图片缓存
	...
	public function actionUpdate($id)
	{
		...
		$model=$this->loadModel($id,array(
			'condition'=>'Store_Agent.status=1 AND Play_Items.status=0 AND t.c_id=:c_id AND Play_Items.audit !=:audit',
			'with'=>array(
				'Play_Items'=>array(
						'with'=>array(
		...
		if((isset($_POST['add']) || isset($_POST['cut'])) && isset($_POST['Items']) && isset($_POST['Fare']) && is_array($_POST['Fare']))
		{
			if(isset($_POST['Items']['free_status']) && $_POST['Items']['free_status'] != Items::free_status_yes)
			{
	            ...
	            $array=$this->set_attributes($model->Play_Fare,$default,$number);//设置属性 保存数据
	            if($number > $count )
	            {
		...
		if(!isset($_POST['add']) && !isset($_POST['cut']) && isset($_POST['Items']) && isset($_POST['Fare']) && is_array($_POST['Fare']) && count($_POST['Fare'])==count($model->Play_Fare))
		{
			...
			$model->Play_Items->attributes=$_POST['Items'];//itmes
			//项目字段验证
				...
				try
				{
					...
					if($model->Play_Items->save(false))//保存项目主要表
					{
						...
						if(((string)$old_lng != (string)$model->Play_Items->lng || (string)$old_lat != (string)$model->Play_Items->lat))
						{
							...
							if(!isset($Fare_is_error) && ((isset($img_is_error) && $img_is_error==false) || !isset($img_is_error)))
							{
								$return =$this->log('修改项目(玩)',ManageLog::admin,ManageLog::create);
							}else
								throw new Exception("修改项目玩记录错误");
					}else
	...
	public function actionSelect($id){
		...
		$this->render('select',array(
				...
				'select'=>$this->loadModel($id,array(
						'with'=>array('Play_Items'),
						'condition'=>'Play_Items.status=0 AND Play_Items.audit !=:audit AND Play_Items.c_id=:c_id',
	...
	public function actionTags($id){
		if(isset($_POST['tag_ids']) && $_POST['tag_ids'] && isset($_POST['type']))
		{
			...
			if($type=='yes'){
				//过滤之前有的
				$save_tags_id=TagsElement::not_select_tags_element($tags_ids,$id,TagsElement::tags_items_play);							
				$return=TagsElement::select_tags_ids_save($save_tags_id, $id, TagsElement::tags_items_play);
			}else
				$return=TagsElement::select_tags_ids_delete($tags_ids, $id, TagsElement::tags_items_play);		
			if($return){
				if($type=='yes')
					$this->log('项目(玩)添加标签', ManageLog::admin,ManageLog::create);
				else
					$this->log('项目(玩)去除标签', ManageLog::admin,ManageLog::clear);
				echo 1;
			}else
	...
	public function actionRestore($id)
	{
		...
		if($this->loadModel($id,'`status`=-1 AND `c_id`=:c_id',array(':c_id'=>$c_id))->updateByPk($id,array('status'=>0)))
			$this->log('还原项目(玩)',ManageLog::admin,ManageLog::update);
	...
	public function actionPass($id){
		...
		//查看是否需要审核
		$model=$this->loadModel($id,array(
				'with'=>array('Play_Items'=>array(
						'condition'=>'Play_Items.status=0 AND Play_Items.audit=:audit AND t.c_id=:c_id',
						'params'=>array(':audit'=>Items::audit_pending,':c_id'=>ItemsClassliy::getClass()->id),
				))
		));
		$model->Play_Items->scenario='pass';
		$this->_Ajax_Verify($model->Play_Items,'items-form');

		if(isset($_POST['Items']))
		{
			$model->Play_Items->attributes=$_POST['Items'];
			$model->Play_Items->pub_time=time();
			$model->Play_Items->is_push=Items::push_init;//设置初始化
			$model->Play_Items->audit=Items::audit_pass;// 审核通过
			$transaction=$model->dbConnection->beginTransaction();
			if($model->Play_Items->validate()){
				try
				{
					if($model->Play_Items->save(false)){
						$audit=new AuditLog;
						$audit->info=$model->Play_Items->push.'%'.$model->Play_Items->push_orgainzer.'%'.$model->Play_Items->push_store.'%'.$model->Play_Items->push_agent.'%';
						$audit->audit_who=AuditLog::admin;//记录 审核人 （审核人的id 在模型中）
						$audit->audit_element=AuditLog::items_play;//记录 被审核的类型
						$audit->element_id=$model->id;//记录 被审核id
						$audit->audit=AuditLog::pass;//记录 审核通过
						if($audit->save(false))
							$return=$this->log('添加审核项目(玩)记录分成比例',ManageLog::admin,ManageLog::create);
						else
	...
	public function actionNopass($id){
		...
		//查看是否需要审核
		$model->Audit_Play=$this->loadModel($id,array(
				'with'=>array('Play_Items'=>array(
						'condition'=>'Play_Items.status=0 AND Play_Items.audit=:audit AND t.c_id=:c_id',
						'params'=>array(':audit'=>Items::audit_pending,':c_id'=>ItemsClassliy::getClass()->id),
				))
		...
		if(isset($_POST['AuditLog']))
		{
			...
			$model->audit_element=AuditLog::items_play;//记录 被审核的
			$model->element_id=$model->Audit_Play->id;//记录 被审核id
			...
			if($model->validate()){
				...
				try
				{
					if($model->save(false))
					{
						$model->Audit_Play->Play_Items->audit=Items::audit_nopass;//审核不通过
						if($model->Audit_Play->Play_Items->save(false))
							$return=$this->log('项目(玩)审核不通过记录',ManageLog::admin,ManageLog::create);
					}else
						throw new Exception("添加项目(玩)审核不通过日志错误");
...
// 		try
// 		{
// 			if($model->delete() && $model->Play_Items->delete())
// 			{
// 				$this->upload_delete($model->Play_Items->map);
// 				foreach ($model->Play_ItemsImg as $model_img)
// 				{
// 					if($model_img->delete())
// 						$this->upload_delete($model_img->img);
// 					else
// 						throw new Exception("删除项目(玩)图片记录出现错误");
// 				}
// 				foreach ($model->Play_Fare as $model_fare){
// 					if(! $model_fare->delete())
// 						throw new Exception("删除项目(玩)价格记录出现错误");			
// 				}		
// 				$this->log('彻底删除项目(玩)',ManageLog::admin,ManageLog::delete);
// 			}else
// 				throw new Exception("添加项目(玩)中出现记录错误");
// 			$transaction->commit();
// 		}
...
// 	}
	...
	public function actionIndex()
	{
		...
		$criteria->addColumnCondition(array('Play_Items.status'=>-1));
		$model=new Play;	
	...
	public function actionAdmin()
	{
		...
		$model->unsetAttributes();  // 删除默认属性
		$model->Play_Items=new Items('search');
		$model->Play_Items->unsetAttributes();  // 删除默认属性
		if(isset($_GET['Play']))
			$model->attributes=$_GET['Play'];
		if(isset($_GET['Items']))
			$model->Play_Items->attributes=$_GET['Items'];
	...
	public function actionDisable($id){
		...
		$this->_class_model='Items';	
		if($this->loadModel($id,'`status`=1 AND `c_id`=:c_id',array(':c_id'=>$c_id))->updateByPk($id,array('status'=>0)))
			$this->log('禁用项目(玩)',ManageLog::admin,ManageLog::update);			
		if(!isset($_GET['ajax']))
			$this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : $this->back(true));
	}
	...
	public function actionStart($id){
		...
		if($this->loadModel($id,'`status`=0 AND `c_id`=:c_id AND `audit`=:audit',array(':c_id'=>$c_id,':audit'=>Items::audit_pass))->updateByPk($id,array('status'=>1)))
	 		$this->log('激活项目(玩)',ManageLog::admin,ManageLog::update);
		if(!isset($_GET['ajax']))
			$this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : $this->back(true));
	}
	...
	public function actionConfirm($id)
	{
		...
		if($this->loadModel($id,'`status`=0 AND `c_id`=:c_id AND audit=:audit',array(':c_id'=>$c_id,':audit'=>Items::audit_draft))->updateByPk($id,array('audit'=>Items::audit_pending)))
			$this->log('提交项目(玩)审核',ManageLog::admin,ManageLog::update);
