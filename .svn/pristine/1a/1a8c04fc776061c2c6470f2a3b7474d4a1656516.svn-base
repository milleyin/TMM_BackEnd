...
class Actives extends CActiveRecord
{
	 ...
	 * @var array
	 */
	public static $_actives_status=array(-1=>'已取消',0=>'未开始',1=>'报名中',2=>'报名结束');
	...
	*/
	 ...
	 * @var array
	 */
	public static $_tour_type=array(-1=>'农产品觅趣','多个点','一条线');
	 ...
	 * @var array
	 */
	public static $_status=array(-1=>'未发布','已结束','进行中');
	/**
	 * 搜索的时间类型
	 * @var array
	 */
	...
	public function rules()
	{
		...
		//	array('id, c_id, organizer_id, remark', 'required'),
			...
			array('remark', 'length', 'max'=>2000,'on'=>'create,create_thrand,api_update_actives_remark'),
	...
	public function search($criteria='')
	{
		...
		if($criteria ===''){
			...
			$criteria->with=array(
//				'Actives_Organizer'=>array('with'=>array(
//					'Organizer_User',
//				)),
				'Actives_User'=>array('with'=>array(
					'User_Organizer'
				)),
			...
			//$criteria->compare('Actives_Organizer.firm_phone',$this->organizer_id,true);
			$criteria->compare('Actives_User.phone',$this->organizer_id,true);
			$criteria->compare('Actives_Shops.audit',$this->Actives_Shops->audit,true);
			...
			$criteria->compare('Actives_Shops.is_sale',$this->Actives_Shops->is_sale,true);
			$criteria->compare('Actives_Shops.selected',$this->Actives_Shops->selected,true);
			$criteria->compare('Actives_Shops.tops',$this->Actives_Shops->tops,true);
			$criteria->compare('Actives_Shops.selected_tops',$this->Actives_Shops->selected_tops,true);
			...
			$criteria->compare('t.is_open',$this->is_open,true);
			$criteria->compare('t.is_organizer',$this->is_organizer,true);
...
// 		else if (!$this->hasErrors('go_time') && $this->go_time && $this->go_time <= date('Y-m-d'))
// 			$this->addError('go_time', '出游时间必须大于今天');
	}
