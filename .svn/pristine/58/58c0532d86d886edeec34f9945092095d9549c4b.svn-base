...
class AccountLog extends CActiveRecord
{
	...
	public static $_account_type=array(
			self::account_agent=>'运营商',
			self::account_store=>'供应商',
	...
	*/
	 ...
	 */
	  ...
	  * @var integer
	  */
	 const funds_type_entry_order_refund_penalty=16;
	  ...
	  * @var integer
	  */
	 const funds_type_record_order_penalty=42;
	 ...
	 public static $__funds_type=array(
			...
			//1.2.7
	 		self::funds_type_entry_order_refund=>'EOR',									//入 订单退款 (用户)
	 		self::funds_type_entry_order_refund_penalty=>'EORP',					//入 违约收益 （平台）
	 		
	 		self::funds_type_pending_cash_frozen=>'PCF',					// 冻结 只有成功的 => 作： 记录 ,否则 => 作： 出 提现申请
	 		...
	 		//1.2.7
	 		self::funds_type_record_order_penalty=>'ROP',				//记录 订单违约 （用户）
	 );
	 /***************************************** 核心状态 ***************************************************************/
	 ...
	 */
	  ...
	  * @var integer
	  */
	 const info_type_entry_order_refund_penalty = self::funds_type_entry_order_refund_penalty;
	 ...
	 public static $_info_type=array(	
			...
			self::info_type_entry_actives_tour_income=>'觅趣收益',
	 		...
	 		self::info_type_entry_order_refund_penalty=>'违约收益', //平台
	 ...
	 public static $__info_type=array(
	 	...
	 	//	self::info_type_entry_recharge=>'AccountLog_Recharge',   											//待添加
	 		...
	 		//1.2.7
	 		self::info_type_entry_order_refund=>'AccountLog_Order',
	 		self::info_type_entry_order_refund_penalty=>'AccountLog_Order',
	 		...
	 		//1.2.7
	 		self::info_type_record_order_penalty=>'AccountLog_Order',
	 );
	 /**
	  * 详情ID 没有详情ID
	  * @var unknown
	 ...
	 public static $_info_name=array( 		
	 		...
	 		//1.2.7
	 		self::info_type_entry_order_refund=>'order_no',							//已添加
	 		self::info_type_entry_order_refund_penalty=>'order_no',			//已添加
	 		...
	 		//1.2.7
	 		self::info_type_record_order_penalty=>'order_no',
	 );
	...
	public function rules()
	{
		...
		return array(
			...
			//operatorSearch
			array('account_no, to_account_id, to_account_type, funds_type, money_type, use_money, money, after_money, name, add_time, log_status, centre_status', 'safe', 'on'=>'operatorSearch'),			
		);
	}
...
// 				'AccountLog_Recharge'=>array(self::BELONGS_TO,'Recharge','info_id'),						//待添加
	...
	public function attributeLabels()
	{
		return array(
			...
			'account_id' => '当前账户',
			'account_type' => '角色类型',
			'to_account_id' => '来自/去向账户',
			'to_account_type' => '来自/去向账户类型',
	...
	public function operatorSearch($criteria='')
	{
		...
		if ($criteria === '')
		{
			...
			$criteria->addColumnCondition(array(
					'`t`.`account_id`'=>Yii::app()->operator->id,
					'`t`.`account_type`'=>self::account_agent,
			));
			$criteria->compare('t.account_no', $this->account_no,true);		
			//操作给谁
			$criteria_to = false;
