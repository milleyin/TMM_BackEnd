...
/**
	 ...
	 * @var string
	 */
	public static $_search_time_type = array('<?php echo isset($search_time_type)?implode("', '", $search_time_type):''?>'); 
	/**
	 *	解释搜索类型时间 search_time_type 的字段搜索
	 * @var string
	 */
	public $__search_time_type = array('<?php echo isset($search_time_type)?implode("', '", $search_time_type):''?>'); 
	/**
	 * 搜索开始的时间
	 * @var string
	 */
...
<?php endforeach; ?>
			...
			array('search_time_type, search_start_time, search_end_time, <?php echo implode(', ', array_keys($columns)); ?>', 'safe', 'on'=>'search'),
		);
	}
...
<?php endforeach; ?>
	...
	public function search($criteria='')
	{
		...
		if ($criteria === '')
		{
			...
			<?php if(isset($status)){?>if ($this->status != self::status_del)
...
{	
	...
	{	
		if($name=='add_time')
			$add_time=true;
		echo "\t\t\tif (\$this->$name != '')\n";
		echo "\t\t\t\t\$criteria->addBetweenCondition('$name', strtotime(\$this->$name), (strtotime(\$this->$name)+3600*24-1));\n";
