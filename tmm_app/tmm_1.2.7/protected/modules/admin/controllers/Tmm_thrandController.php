...
class Tmm_thrandController extends MainController
{
	...
	{	
		$this->addCss(Yii::app()->baseUrl.'/css/admin/main/right/thrand/view.css');
		//条件
	...
	public function actionIndex()
	{
		...
		$criteria->addColumnCondition(array('Thrand_Shops.status'=>Shops::status_del));
	...
	{
		$this->addCss(Yii::app()->request->baseUrl . '/css/admin/main/right/thrand/form.css');
		$this->addCss(Yii::app()->request->baseUrl . '/css/admin/main/right/thrand/viewdot.css');
		//实例化 线路
		$model = new Thrand;
		//商品表
		...
		$model->Thrand_Shops->Shops_Agent = $this->loadModel($id, 'status=:status', array(':status'=>Agent::status_suc));
		//数据转换	
			...
			if ($validate_shops && $validate_pro && $validate_profare)
			{
				...
				try
				{
					...
					foreach ($model->Thrand_Pro as $pro_save)
					{
 					...
 					//继承点的tags
					TagsElement::select_tags_ids_save(TagsElement::select_tags_all($element_ids), $model->id, TagsElement::tags_shops_thrand, TagsElement::admin);
					//日志
					$return = $this->log('创建线路', ManageLog::admin, ManageLog::create);
					$transaction->commit();
				}
				catch(Exception $e)
				{
		...
		if (isset($_POST['Shops']) && !$model->Thrand_Shops->hasErrors())
		{
			...
			if ($validate_shops && $validate_pro && $validate_profare)
			{
				...
				try
				{
					...
					//日志
					$return = $this->log('修改线路', ManageLog::admin, ManageLog::update);
					$transaction->commit();
				}
				catch(Exception $e)
				{
