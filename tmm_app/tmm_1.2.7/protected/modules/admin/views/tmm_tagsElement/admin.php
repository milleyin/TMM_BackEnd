...
$('.search-form form').submit(function(){
	$('#tags-element-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
");
?>
<h1>管理页 Tags Elements</h1>
<div>
	</span>
		<?php echo CHtml::link('高级搜索','#',array('class'=>'search-button')); ?>	</span>
	 <span>
	 	<?php echo CHtml::link('创建TagsElement',array('create')); ?>	</span>
</div>
...
function(){ 
	...
	$.fn.yiiGridView.update('tags-element-grid',{  
	type:'POST',
...
function() {  
	...
	jQuery('#tags-element-grid').yiiGridView('update', {
		type: 'POST',
		...
		array(
			...
			'buttons'=>array(
					...
					'disable'=>array(
						...
						'url'=>'Yii::app()->createUrl("/admin/tmm_tagsElement/disable",array("id"=>$data->id))',
						'click'=>$click,
						'options'=>array('style'=>'padding:0 8px 0 0;'),
					...
					'start'=>array(
						'label'=>'激活',
						'visible'=>'$data->status==0',
						'url'=>'Yii::app()->createUrl("/admin/tmm_tagsElement/start",array("id"=>$data->id))',
						'click'=>$click,
						'options'=>array('style'=>'padding:0 8px 0 0;'),
