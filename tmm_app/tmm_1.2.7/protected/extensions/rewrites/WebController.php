...
class WebController extends SBaseController
{
	 ...
	 * @param unknown $Id
	 */
	public function _Ajax_Verify($models, $id='', $tabular=false)
	{
		if(isset($_POST['ajax']) && ($_POST['ajax'] === $id))
		{
			if ($tabular)
				$this->_Ajax_Verify_Same($models, $id);
			else
	...
	{
		if($this->file_exists_uploads($path_name))
		{
			...
			foreach (Yii::app()->params['litimg'] as $key=>$value)
			{
				if( isset(Yii::app()->params['litimg_confing'][$key]) && ($type==''?true:(is_array($type)?in_array($key, $type):$key==$type)) )
				{
					...
					$thumb->width = Yii::app()->params['litimg_confing'][$key]['width'];
	...
	public function _Ajax_Verify_Same($models, $id='')
	{
		if (isset($_POST['ajax']) && ($_POST['ajax'] == $id))
		{
			...
			foreach ($models as $model)
			{
				...
				if (isset($_POST[$name][0]) && is_array($_POST[$name][0]))
					$tabularModels[] = $model;
				else
					$newModels[] = $model;
			}
			$tabularResult = json_decode(CActiveForm::validateTabular($tabularModels), true);
			$result = json_decode(CActiveForm::validate($newModels), true);
			$result = array_merge($result, $tabularResult);
	...
	* @return multitype:
	*/
	public function update_models($models, $number, $scenario, $modelName='')
	{
		$new_models = array();
		foreach ($models as $model)
		{
		...
		$modelName = isset($model) && $model ? CHtml::modelName($model) : $modelName;
		return array_merge($new_models,$this->new_modes($modelName, $scenario,$number));
	}
	...
	*/
	public function set_attributes($models,$default='',$number='',$data=array())
	{
		$new_models=array();
		if (empty($data))
			$data = $_POST;
		if(is_array($models))
		{
			...
			foreach ($models as $key=>$model)
			{
				if($number>0)
				{
					$model->attributes=isset($data[$modelName][$key])?$data[$modelName][$key]:$default;
					$new_models[]=$model;
				}
				$number--;
			}
		}else
			return $models->attributes=$data[CHtml::modelName($models)];
		return $new_models;
	}
	...
	* @return boolean
	*/
	public function models_validate($models, $data=array())
	{
		$validate=array();
		$this->set_attributes($models, $default='', $number='', $data);
		foreach ($models as $model)
		{
			if($model->validate())
				$validate[]=true;
		}
		return count($models)==count($validate) && !empty($validate);
	}
	...
	*/
	public function show_img($path,$type='',$alt='',$params=array(),$click=true)
	{
		...
		if($alt=='')
			$alt=$this->rewritePath($litimg_path);		
		if($click)
		{
			if (isset($params['id']))
				$id = $params['id'];
			else 
				$id = 'show_original_img';
			$this->showOriginal($id);
			$html = CHtml::image($this->rewritePath($path),$this->rewritePath($path),array(
					'style'=>'min-width:90%;cursor:pointer;',
					'onclick'=>'$("#'.$id.'").dialog("close"); return false;'
			));
			if(isset($params['onclick']))
				$params['onclick'] .= '$("#'.$id.'").dialog("open").html(\''.$html.'\'); return false;';
			else
				$params['onclick'] = '$("#'.$id.'").dialog("open").html(\''.$html.'\'); return false;';
		...
		return CHtml::image($this->rewritePath($litimg_path),$this->rewritePath($litimg_path),$params);
	}
	...
	{
		if (! isset($this->isShowOriginal[$id]))
		{
			ob_start();
