...
class Dot extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			//创建线路
			array('id', 'safe', 'on'=>'createThrandSearch'),			
		);
	}
	 ...
	 */
	public function createThrandSearch($criteria='')
	{
		// @todo Please modify the following code to remove attributes that should not be searched.
		if($criteria ==='')
		{
		...
		return new CActiveDataProvider($this, array(
				...
				'sort'=>array(
						...
						'attributes'=>array(
								...
								'Dot_Shops.is_sale'=>array(
										'desc'=>'Dot_Shops.is_sale desc',
								),
						)
				),
		));
								...
								foreach ($items_id_fare_key as $items_id=>$fare_key_fare_id)//
								{
									...
									foreach ($fare_key_fare_id as $fare_key=>$fare_id_price_number)
									{
										...
										foreach ($fare_id_price_number as $fare_id=>$price_number)
										{
											...
											if(! $floor->isNumeric($price_number['number']) || $price_number['number'] <= 0 || $floor->floorComp($price_number['price'], $fare_model->price) != 0)
											{
												$model->addError('order_price','订单中商品 不是有效值');
