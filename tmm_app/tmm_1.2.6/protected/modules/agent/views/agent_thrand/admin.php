	...
	<div class="content_div">
		<ul class="breadcrumb my_breadcrumb">
			<li class="bottom_line">
				<?php echo CHtml::link('我的线'.($count['thrand']!=0?'（<div class="num">'.$count['thrand'].'</div>）':''), array('/agent/agent_thrand/admin'), array('target'=>'content')); ?>
				<span></span>
			</li>
			<li>
				<?php echo CHtml::link('审核未通过'.($count['nopass']!=0?'（<div class="num">'.$count['nopass'].'</div>）':''), array('/agent/agent_thrand/admin_no_pass'), array('target'=>'content')); ?>
				<span></span>
			</li>
			<li>
				<?php echo CHtml::link('草稿'.($count['draft']!=0?'（<div class="num">'.$count['draft'].'</div>）':''), array('/agent/agent_thrand/admin_draft'), array('target'=>'content')); ?>
				<span></span>
			</li>
		</ul>
		<div class="main_div">
				...
				<div class="controls controls-row  query" id="items_search">
					...
					?>
						...
						<div class="input-append element_group span3">
							<input type="text" name="search_info" placeholder="线路名称" class="span3">
							<?php echo CHtml::submitButton('',array('class'=>'add-on element_icon search', 'style'=>'z-index:9999;'));?>
						</div>
					<?php $this->endWidget(); ?>
					<?php echo CHtml::link('创建线',array('/agent/agent_thrand/create'),array('class'=>'create pull-right'))?>
				</div>  <!-- .query -->
				<!-- .main_content -->
				<div class="main_content data">
<?php
Yii::app()->clientScript->registerScript('search', "
jQuery('#items_search form').submit(function(){
...
EOD;
$start_click=<<<EOD
jQuery(document).on('click','#items_list a.start',function() {
    if(!confirm('确定要上线此线路吗?')) return false;
    var th = this,
        afterDelete = function(){};
    $.fn.yiiListView.update('items_list', {
        type: 'POST',
...
EOD;
$disable_click=<<<EOD
jQuery(document).on('click','#items_list a.disable',function() {
    if(!confirm('确定要下线此线路吗?')) return false;
    var th = this,
        afterDelete = function(){};
    $.fn.yiiListView.update('items_list', {
        type: 'POST',
...
Yii::app()->clientScript->registerScript('start', $start_click);
Yii::app()->clientScript->registerScript('disable', $disable_click);
?>
