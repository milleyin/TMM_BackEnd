...
class Account extends CActiveRecord
{
	 ...
	 * @var array
	 */
	public static $_account_type=array(1=>'代理商',2=>'商家',3=>'外部',4=>'用户',5=>'平台',6=>'系统');
	/**
	 * 其他(非系统)ID
	 * @var integer
	 */
	 ...
	 */
 	 ...
 	 * @param unknown $money
 	 */
 	public static function moneyEntryOrderRefundActivesTourCharge($money)
 	{
 		self::$money_attributes=array(
 				'money'=>new CDbExpression('`money`+:money',array(':money'=>$money)),
 				'no_money'=>new CDbExpression('`no_money`-:no_money',array(':no_money'=>$money)),
 	 ...
 	 * @param unknown $money
 	 */
 	public static function moneyEntryOrderCompensate($money)
 	{
 		self::$money_attributes=array(
 				'total'=>new CDbExpression('`total`+:total',array(':total'=>$money)),
 				'count'=>new CDbExpression('`count`+:count',array(':count'=>$money)),
 	 ...
 	 * @param unknown $money
 	 */
 	public static function moneyEntryActivesTourCompensate($money)
 	{
 		self::$money_attributes=array(
 				'total'=>new CDbExpression('`total`+:total',array(':total'=>$money)),
 				'count'=>new CDbExpression('`count`+:count',array(':count'=>$money)),
 	 ...
 	 * @param unknown $money
 	 */
 	public static function moneyRecordOrderRefundFrozen($money)
 	{
 		self::$money_attributes=array(
 				'no_money'=>new CDbExpression('`no_money`+:no_money',array(':no_money'=>$money)),
 				'not_consume_count'=>new CDbExpression('`not_consume_count`-:not_consume_count',array(':not_consume_count'=>$money)),
 	 ...
 	 * @param unknown $money
 	 */
 	public static function moneyRecordOrderRefundFail($money)
 	{
 		self::$money_attributes=array(
 				'no_money'=>new CDbExpression('`no_money`-:no_money',array(':no_money'=>$money)),
 				'not_consume_count'=>new CDbExpression('`not_consume_count`+:not_consume_count',array(':not_consume_count'=>$money)),
 	 ...
 	 * @param unknown $money
 	 */
 	public static function moneyRecordActivesTourPenalty($money)
 	{
 		self::$money_attributes=array(
 				'total'=>new CDbExpression('`total`-:total',array(':total'=>$money)),
 				'no_money'=>new CDbExpression('`no_money`-:no_money',array(':no_money'=>$money)),
	...
	AccountLog::funds_type_entry_refund_actives_tour_charge=>'moneyEntryRefundActivesTourCharge',	//入 服务费用退款	 	
		 		AccountLog::funds_type_entry_order_compensate=>'moneyEntryOrderCompensate',					//入 订单补偿
		 	AccountLog::funds_type_entry_actives_tour_compensate=>'moneyEntryActivesTourCompensate',//入	服务费用补偿
 				...
 				//1.2.6
 				AccountLog::funds_type_record_order_refund_frozen=>'moneyRecordOrderRefundFrozen',		//记录	退款申请
 				AccountLog::funds_type_record_order_refund_fail=>'moneyRecordOrderRefundFail',					//记录	退款失败
	 			...
	 			AccountLog::funds_type_record_actives_tour_penalty=>'moneyRecordActivesTourPenalty',   		//记录 服务费用违约
 		);
 	 */
 	 ...
 	 * //1.2.6
 	 * 退款申请
 	 * @param unknown $money 退款金额
 	 * @param unknown $who_role array(account_id=>'', account_type=>Account::user) 用户id
 	 * @param unknown $info	 $info['info_id'],$info['info'],$info['name']
 	 * @return boolean|Ambigous <multitype:, boolean, number, unknown>
 	 */
 	public static function moneyRecordOrderRefundFrozenRmb($money,$who_role,$info)
 	{
 		$money_type=self::money_type_rmb;
 		$manage = array();
 		if (isset(Yii::app()->api,Yii::app()->api->id) && Yii::app()->api->id)
 		{
 			$manage['manage_account_id'] = Yii::app()->api->id;
 			$manage['manage_account_type'] = AccountLog::manage_account_user;
 		}
 		else if (isset(Yii::app()->admin,Yii::app()->admin->id) && Yii::app()->admin->id)
 		{
 			$manage['manage_account_id'] = Yii::app()->admin->id;
 			$manage['manage_account_type'] = AccountLog::manage_account_admin;
 		}
 		else 
 			return false;
 		if(self::isMoney($money) && isset($who_role['account_id'],$who_role['account_type'],$info['info_id'],$info['info'],$info['name']))
 		{
 			//退款申请
 			$info['info_type']=AccountLog::info_type_record_order_refund_frozen;
 			$old_model=self::getAccount($who_role['account_id'],$who_role['account_type'],$money_type);
 			//退款申请
 			self::moneyRecordOrderRefundFrozen($money);
 			//提现申请 执行钱包
 			if($old_model && self::moneyExecuteByPk($old_model->id))
 			{
 				$new_model=self::model()->findByPk($old_model->id);
 				return AccountLog::createLog(
 						...
 						$money,
 						//操作类型 退款申请
 						AccountLog::funds_type_record_order_refund_frozen,
 						//原来的
 						self::getAccountArray($old_model),
 						//更新后的
 						self::getAccountArray($new_model,'after_'),
 	 ...
 	 * //1.2.6
 	 * 退款失败	需要管理员登录
 	 * @param unknown $money  		退款失败
 	 * @param unknown $who_role	退款失败的人
 	 * @param unknown $info				退款失败的详情  需要申请表的ID
 	 * @return Ambigous <multitype:, boolean, number, unknown>|boolean
 	 */
 	public static function moneyRecordOrderRefundFailRmb($money,$who_role,$info)
 	{
 		$money_type=self::money_type_rmb;
 		$admin_id = isset(Yii::app()->admin,Yii::app()->admin->id) ? Yii::app()->admin->id : 0;
 	
 		if(self::isMoney($money) && $admin_id && isset($who_role['account_id'],$who_role['account_type'],$info['info_id'],$info['info'],$info['name']))
 		{
 			//退款失败
 			$info['info_type'] = AccountLog::info_type_record_order_refund_fail;
 			//原来的
 			$old_model=self::getAccount($who_role['account_id'],$who_role['account_type'],$money_type);
 			//退款失败
 			self::moneyRecordOrderRefundFail($money);
 			//执行 退款申请
 			if ($old_model &&
 					//执行钱包操作
 					...
 					AccountLog::setLogStatus(
 							...
 							$who_role['account_id'],
 							//账户
 							$who_role['account_type'],
 							$info['info_id'],
 							//退款申请
 							 AccountLog::info_type_record_order_refund_frozen,
 							//处理失败
 							AccountLog::log_status_fail
 					)
 			)
 			{
 				$new_model=self::model()->findByPk($old_model->id);
 				return AccountLog::createLog(
 						...
 						$money,
 						//操作类型 退款失败
 						AccountLog::funds_type_record_order_refund_fail,
 						//原来的
 						self::getAccountArray($old_model),
 						//更新后的
 						self::getAccountArray($new_model,'after_'),
 	...
 	{
 		if(empty($attributes))
 			$attributes=array('count', 'total', 'money', 'no_money', 'cash_count', 'pay_count', 'refund_count', 'consume_count', 'not_consume_count');
 		$return=array();
