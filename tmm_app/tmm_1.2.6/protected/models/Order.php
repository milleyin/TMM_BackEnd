...
{	
	 ...
	 * @var unknown
	 */
	public static $_pay_type=array(1=>'支付宝',2=>'钱包');
...
// 	const order_type_farm=6;
	 ...
	 * @var unknown
	 */
	public static $_order_type=array(1=>'自助游(点)',2=>'自助游(线)',3=>'觅趣(自费)',4=>'觅趣(代付)');
...
//			}
			...
			//当前代理商
			//$criteria->compare('Order_OrderShops.shops_agent_id','='.Yii::app()->agent->id);
	...
	public function search_dot_thrand($criteria='')
	{
		...
		if($criteria ===''){
			...
			$criteria->addColumnCondition(array(
				'`Order_OrderShops`.`shops_agent_id`'=>Yii::app()->agent->id,
			));
...
//			}
	...
	public static  function my_income_price($order_id)
	{
		...
		return  Yii::app()->controller->money_floor($item_total_money,2,'floor');
	}
	/**
	 * 获取唯一的订单号
	 * @param unknown $id
	...
	public static function alipay($model)
	{
		...
		$alipay_config['body']=$model->order_no;				//商品详情 
	...
	public static function validate_order($model)
	{
		if(isset($_POST['Order']) && isset($_POST['Order']['order_price']) && isset($_POST['Order']['order_type']))
		{
			...
			{
				if(bccomp($money['price'] * $money['number'],$money['total'],2)==0)
					$money_count += $money['total'];
			}
			if(bccomp(Order::$_money_count,$money_count,2))
			{
				$model->addError('order_price','订单中价格 不可空白');
			...
			elseif($model->order_type == Order::order_type_actives_tour)//活动旅游
			{
				...
				if($model->user_price != (Order::$retinue_count*Order::$actives_tour_price))
				{
					$model->addError('order_price','订单数据 不可空白');
					$model->addError('status','AT01');
					return false;
				}
				if($model->user_price != ($model->user_go_count * Order::$actives_tour_price))
				{
					$model->addError('order_price','订单数据 不可空白');
					$model->addError('status','AT01');
					return false;
				}
				$model->user_price_fact=$model->user_price=Order::$actives_tour_price;//服务费
				Order::$buy_number=Order::$retinue_count;
			}else
				return false;
			$model->money_count=(string)(Order::$_money_count+$model->user_price_fact*$model->user_go_count);//订单总计
			if(! $model->validate())
				return false;
			if($return)
				return true;
	 ...
	 * @return boolean
	 */
	public static function pay_status_paying($order_id)
	{
		...
		if($model)
			return self::model()->updateByPk($order_id, array('pay_status'=>Order::pay_status_paying),$criteria);
	...
	public static function pay_status_not($order_id)
	{
		...
		$model=self::model()->findByPk($order_id,$criteria);
		if($model)
			return self::model()->updateByPk($order_id, array('pay_status'=>Order::pay_status_not),$criteria);
	 ...
	 * @return string
	 */
	public static function money_floor($money,$int=2,$type='floor')
	{
	...
	public function order_price()
	{
		if(! $this->hasErrors('order_price'))
		{
			...
			$total += $number * $this->user_price_fact;
			if (self::money_floor($total) !== self::money_floor($this->order_price))
			{
