...
$('.search-form form').submit(function(){
	$('#items-wifi-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
");
?>
<h1>管理页 Items Wifis</h1>
<div>
	</span>
		<?php echo CHtml::link('高级搜索','#',array('class'=>'search-button')); ?>	</span>
	 <span>
	 	<?php echo CHtml::link('创建ItemsWifi',array('create')); ?>	</span>
</div>
...
function(){ 
	...
	$.fn.yiiGridView.update('items-wifi-grid',{  
	type:'POST',
...
function() {  
	...
	jQuery('#items-wifi-grid').yiiGridView('update', {
		type: 'POST',
		...
		array(
			...
			'buttons'=>array(
					...
					'disable'=>array(
						...
						'url'=>'Yii::app()->createUrl("/admin/tmm_itemsWifi/disable",array("id"=>$data->id))',
						'click'=>$click,
						'options'=>array('style'=>'padding:0 8px 0 0;'),
					...
					'start'=>array(
						...
						'url'=>'Yii::app()->createUrl("/admin/tmm_itemsWifi/start",array("id"=>$data->id))',
						'click'=>$click,
						'options'=>array('style'=>'padding:0 8px 0 0;'),
