...
$data_array=array();
$info_array=array();
//$this->p_r($model->Order_OrderShops);
if(isset($model->Order_OrderShops[0]->OrderShops_OrderItems))
{
		foreach ($model->Order_OrderShops[0]->OrderShops_OrderItems as $value)
		{
        ...
        foreach($data_array as $k=>$vs) {
            ...
            <div class="group_wrap">
                <span class="warp_day"><?php echo CHtml::encode(Pro::item_swithc($k)); ?></span>
                        ...
                        <?php
                        foreach ($vs as $key_v=>$v) {
                        ?>
                            <div class="warp_left">
                        <div class="item">
                            <span class="item_id"><?php echo CHtml::encode($key_v + 1); ?></span>
                            ...
                            <div class="item_content">
                                <h3>
                                    ...
                                    &nbsp;&nbsp;消费码：
                                    <span style="width: 60px;" title="<?php echo $v->employ_time==0?'':date('Y-m-d H:i:s',$v->employ_time);?>">
                                        <?php echo CHtml::encode(OrderItems::$_is_barcode[$v->is_barcode]); ?>
                                    </span>
                                    <span style="width: 60px;">
                                        <?php echo CHtml::encode(Items::$_free_status[$v->items_free_status]); ?>
                                    </span>
                                </h3>

                                <div class="store">
                                    <label><strong>所属供应商：</strong></label>
                                    <span title="<?php echo $v->OrderItems_StoreUser->phone;?>">
                                    	<?php echo CHtml::encode($v->OrderItems_StoreUser->Store_Content->name).'('. $v->OrderItems_StoreUser->phone.')';?>
                                    </span>
                                </div>

                                <div class="address">
                                    <label>地址：</label>
                                    <span><?php echo CHtml::encode($v->items_address); ?></span>
                                </div>
                                <div class="address">
                                    <label>接单状态：</label>
                            ...
                            <div class="item_shops">
                                ...
                                <?php 
                                	echo Fare::show_order_organizer_fare($v,$v->items_c_id == 2 ? true : false);
                                ?>
