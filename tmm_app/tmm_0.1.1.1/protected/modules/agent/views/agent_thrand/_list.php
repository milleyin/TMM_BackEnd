<table class="table item_box">
    ...
    <tr>
        <th class="s_head">
            ...
            <?php if ($data->Thrand_Shops->audit == Shops::audit_nopass && $data->Thrand_Shops->status == Shops::status_offline) { ?>
                <div class="pull-right mark mark1"><span>审核未通过</span></div>
            <?php } elseif($data->Thrand_Shops->audit == Shops::audit_pending && $data->Thrand_Shops->status == Shops::status_offline) { ?>
                <div class="pull-right mark"><span>未审核</span></div>
            <?php } elseif($data->Thrand_Shops->audit == Shops::audit_pass && $data->Thrand_Shops->status == Shops::status_offline) { ?>
                <div class="pull-right mark mark3"><span>未上线</span></div>
            <?php } ?>
        ...
        <td>
            <div class="row-fluid spot">
                ...
                foreach ($data_dot_sort as $data_dot)
                {
                        ...
                        {
                ?>
                        <div class="spot_name"><?php echo CHtml::link(CHtml::encode($info_array['dot_data'][$dot_id]->name),array('/agent/agent_dot/view','id'=>$dot_id)); ?>
                       	<?php 
                            ...
                            <div class="span10 content">
                                <div class="spot_info">
                                    <div class="row-fluid controls controls-row name">
                                        <span class="span5" title="<?php echo CHtml::encode($items->Pro_Items->name)?>">
                                            <?php
...
?>
            ...
            <td>
                <div class="no_online">
                    <div class="row-fluid btn_group">
                        ...
                        <a href="<?php echo Yii::app()->createUrl("/agent/agent_thrand/update",array("id"=>$data->id)); ?>">编辑</a>
                        <?php 
                        	if($data->Thrand_Shops->list_info != '')
							{
                        ?>
                        <a href="<?php echo Yii::app()->createUrl("/agent/agent_shops/start",array("id"=>$data->id)); ?>" class="start">上线</a>
                        <?php 
							}
                        ?>
                    </div>
    ...
    <?php } elseif($data->Thrand_Shops->audit == Shops::audit_pass && $data->Thrand_Shops->status == Shops::status_online) { ?>
        <tr class="last">
            <td>
                <div class="online">
                    <div class="row-fluid btn_group">
                        <a href="<?php echo Yii::app()->createUrl("/agent/agent_shops/disable",array("id"=>$data->id)); ?>" class="disable">下线</a>
                    </div>
                </div>
            </td>
        </tr>
