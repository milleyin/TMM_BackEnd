...
// 			array('search_time_type,search_start_time,search_end_time,id, organizer_id, order_organizer_id, order_id, order_shops_id, shops_day_sort, shops_half_sort, shops_dot_id, shops_thrand_id, employ_time, barcode, is_shops, is_barcode, scan_barcode, add_time, up_time, status', 'unsafe', 'on'=>'create_dot'),
	...
	public static function scancodeDotThrand($model,$past=false)
	{
		...
		foreach ($model->OrderItems_OrderItemsFare as $fare)
		{
			...
			$store['info']['infos'][]= '单项分成：'.$fare->total .' * '.$model->items_push_store / 100;
			...
			$agent['info']['infos'][]= '单项分成：'.$fare->total .' * '.$model->items_push_agent / 100;
			...
			$tmm['info']['infos'][]= '单项分成：'.$fare->total .' - ('.($fare->total .' * '.$model->items_push_agent / 100).' + '.($fare->total .' * '.$model->items_push_store / 100).')';
	
			//计算钱
			$store_money = Order::money_floor($model->items_push_store / 100 * $fare->total);
			$agent_money = Order::money_floor($model->items_push_agent  / 100 * $fare->total);
			//累计
			$store['money'] 	+= $store_money;
			$agent['money'] 	+= $agent_money;
			$tmm['money'] 	+= $fare->total - ($store_money + $agent_money);
		}
	...
	public static function scancodeActivesTour($model,$past=false,$is_organizer=true)
	{
		...
		$store=array(
				...
				'info'=>array('info_id'=>$model->id,'name'=>'订单收益','address'=>'','info'=>'','infos'=>array(
						...
						'项目分类：'.$model->items_c_name,
						'项目总价：'.$model->total,
						'项目分成：'.$model->items_push_store / 100,
				)),
				'to_account'=>array('to_account_id'=>$model->user_id,'to_account_type'=>Account::user),
		);
		$agent=array(
				...
				'info'=>array('info_id'=>$model->id,'name'=>'订单收益','address'=>'','info'=>'','infos'=>array(
						...
						'项目分类：'.$model->items_c_name,
						'项目总价：'.$model->total,
						'项目分成：'.$model->items_push_agent / 100,
				)),
				'to_account'=>array('to_account_id'=>$model->user_id,'to_account_type'=>Account::user),
		);
		if($is_organizer)
		{
			$organizer=array(
					...
					'info'=>array('info_id'=>$model->id,'name'=>'觅趣收益','address'=>'','info'=>'','infos'=>array(
							...
							'项目分类 '.$model->items_c_name,
							'项目总价 '.$model->total,
							'项目分成 '.$model->items_push_orgainzer / 100,
					)),
					'to_account'=>array('to_account_id'=>$model->user_id,'to_account_type'=>Account::user),
			);
		}
		...
		foreach ($model->OrderItems_OrderItemsFare as $fare)
		{
			...
			if($is_organizer)
			{
				...
				$organizer['info']['infos'][]= '单项分成：'.$fare->total .' * '.$model->items_push_orgainzer / 100;
			}
			...
			$tmm['info']['infos'][]= '单项总价：'.$fare->total;
			if($is_organizer)
				$tmm['info']['infos'][]= '单项分成：'.$fare->total .' - ('.($fare->total .' * '.$model->items_push_agent / 100).' + '.($fare->total .' * '.$model->items_push_store / 100).' + '.($fare->total .' * '.$model->items_push_orgainzer / 100).')';
			else
				$tmm['info']['infos'][]= '单项分成：'.$fare->total .' - ('.($fare->total .' * '.$model->items_push_agent / 100).' + '.($fare->total .' * '.$model->items_push_store / 100).')';
			...
			if($is_organizer) 
			{
				$orgainzer_money = Order::money_floor($model->items_push_orgainzer / 100 * $fare->total);
				$organizer['money'] += $orgainzer_money;
				$tmm['money'] += $fare->total - ($store_money + $agent_money + $orgainzer_money);
			}
			else
			{
				$tmm['money'] += $fare->total - ($store_money + $agent_money);
	...
	public static function scancodeActivesTourCharge($models,$past=false)
	{
		...
		{
			$user=array(
					'money'=>$model->Order_OrderActives->user_price * $model->user_go_count,
					'account'=>array('account_id'=>$model->user_id,'account_type'=>Account::user),
