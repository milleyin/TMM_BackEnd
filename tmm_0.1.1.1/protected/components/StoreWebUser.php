 ...
 */
class StoreWebUser extends CWebUser
{
	/**
	 * 加载用户的信息
	 * @var unknown
	...
	{
		if(! $this->_model)
			$this->_model = StoreUser::model()->findByPk($id,'`status`=:status',array(':status'=>StoreUser::status_suc));
	...
	{ 
		...
		{	
			...
			//设置登录状态 //$login_unique
			if($model->set_login(time()))
			{
				//$this->setState(User::login_unique, $login_unique);
	...
	{
		LoginLog::createLog(LoginLog::store,$this);
	}
