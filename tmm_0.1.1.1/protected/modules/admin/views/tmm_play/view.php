...
<h1>查看 项目(玩) <font color='#eb6100'><?php echo CHtml::encode($model->Play_Items->name.
...
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
		...
		array(
				'label'=>'运营商公司名称',
				'name'=>'Play_Items.Items_agent.firm_name',				
				'value'=>$model->Play_Items->Items_agent->firm_name.' [ '.$model->Play_Items->Items_agent->phone.' ]',
		),
		array(
				'label'=>'归属供应商手机',
				'name'=>'Play_Items.Items_StoreContent.Content_Store.phone',				
				'value'=>$model->Play_Items->Items_StoreContent->Content_Store->phone,
		),
		array(
				'name'=>'Play_Items.manager_id',
				'value'=>$model->Play_Items->Items_Store_Manager->phone,
		),
		array(
		...
		array(
				...
				'type'=>'raw',
				'value'=>Push::executed($model->id,'push',$model->Play_Items->push).
				'% （默认分成：'.$model->Play_Items->push.'%）'.
				CHtml::link('查看分成日志',array('/admin/tmm_push/admin','push_id'=>$model->id)),
		),
		array(
				'name'=>'Play_Items.push_agent',
				'type'=>'raw',
				'value'=>Push::executed($model->id,'push_agent',$model->Play_Items->push_agent).
				'% （默认分成：'.$model->Play_Items->push_agent.'%）'.
				CHtml::link('查看分成日志',array('/admin/tmm_push/admin','push_id'=>$model->id)),
		),
		array(
				'name'=>'Play_Items.push_store',
				'type'=>'raw',
				'value'=>Push::executed($model->id,'push_store',$model->Play_Items->push_store).
				'% （默认分成：'.$model->Play_Items->push_store.'%）'.
				CHtml::link('查看分成日志',array('/admin/tmm_push/admin','push_id'=>$model->id)),
		),
		array(
				'name'=>'Play_Items.push_orgainzer',
				'type'=>'raw',
				'value'=>Push::executed($model->id,'push_orgainzer',$model->Play_Items->push_orgainzer).
				'% （默认分成：'.$model->Play_Items->push_orgainzer.'%）'.
				CHtml::link('查看分成日志',array('/admin/tmm_push/admin','push_id'=>$model->id)),
		),
		array(
	...
	if($model->Play_Items->audit==0){
?>
<div class="row">
		<span class="buttons">
	<?php		echo CHtml::ajaxButton('审核通过',array('/admin/tmm_play/pass','id'=>$model->id),
		array(
				'cache'=>true,
		...
		<span class="buttons">
	<?php		echo CHtml::ajaxButton('审核不通过',array('/admin/tmm_play/nopass','id'=>$model->id),
		array(
				'cache'=>true,
				'success'=>'function(html){
					jQuery("#audit").html(html);
