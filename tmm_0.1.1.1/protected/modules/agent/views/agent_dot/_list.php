<table class="table item_box">
	...
	<tr>
		<th class="s_head">
			...
			<?php if ($data->Dot_Shops->audit == Shops::audit_nopass && $data->Dot_Shops->status == Shops::status_offline) { ?>
				<div class="mark mask1"><span>审核未通过</span></div>
			<?php } elseif($data->Dot_Shops->audit == Shops::audit_pending && $data->Dot_Shops->status == Shops::status_offline) { ?>
				<div class="mark mask2"><span>未审核</span></div>
			<?php } elseif($data->Dot_Shops->audit == Shops::audit_pass && $data->Dot_Shops->status == Shops::status_offline) { ?>
			...
			<div class="row-fluid">
				<div class="span8 spot_info">
					<div class="row-fluid controls controls-row name">
						<span class="span5" title="<?php echo CHtml::encode($pro->Pro_Items->name);?>">
						<?php echo CHtml::link(CHtml::encode(mb_substr($pro->Pro_Items->name,0,5,'utf-8').' ...'),array('/agent/agent_'.$pro->Pro_ItemsClassliy->admin.'/view','id'=>$pro->Pro_Items->id)); ?></span>
						<?php 
...
<?php }?>
	...
	<?php } elseif($data->Dot_Shops->audit == Shops::audit_pass && $data->Dot_Shops->status == Shops::status_online) { ?>
	<tr class="last">
		<td>
			<div class="online">
				<div class="row-fluid btn_group">
